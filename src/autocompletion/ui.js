_Define(function(global) {
    ...
    function getCursorPosForTooltip(editor) {
        var place = editor.renderer.getCursorPixelPosition(null,
            true); //position relative to scroller except with css transforms which we avoid
    ...
    UI.prototype.iconClass = function(type) {
        ...
        return suffix ? " " /*remove ace_ prefix*/ + this.iconCls + "completion " + this.iconCls +
            "completion-" + suffix : "";
    };
    ...
    UI.prototype.makeTooltip = function(x, y, content, editor, closeOnCursorActivity, fadeOutDuration,
        onClose) {
    ...
    UI.prototype.renameDialog = function(ts, editor, data) {
        ...
        var tip = this.makeTooltip(null, null, elt('h6', 'tooltip-header', 'Rename ', elt('span',
            "tooltip-name", (data.name || ""))), editor, false);
    ...
    UI.prototype.referenceDialog = function(ts, editor, data) {
        ...
        var container, isTip;
        if (data.refs.length < 4 || window.innerWidth > 720 && window.innerHeight > 720) {
            isTip = true;
            container = this.makeTooltip(null, null, "", null /*not bound to editor*/ , false, -1,
                function() {
                    moveToLastFocus();
                });
            ...
            //use a live list
            sideView.options.focusable = sideView.el.getElementsByTagName('textarea');
            sideView.open();
        }
        ...
        if (isTip) {
            refs.listEl.appendChild(elt('i', 'color-inactive info-tooltip', 'This is not guaranteed to find references in other files or references for non-private variables.'
            )).style.padding = '10px';
        } else {
    ...
    UI.prototype.createToken = function(part, extraClass) {
        return "<span class='doc-" + part.kind + (extraClass ? " doc-" + extraClass : "") + "'>" +
    ...
    function References(container, ts, editor, data) {
        ...
        this.headerText = '<span class="blue-text">' + data.refs.length + '</span>' + " references found" + (
            data.name ?
        ...
        renderer.config.themeClass = "ace_editor " + (editor.renderer.theme.isDark ? "ace_dark " : "") + editor
            .renderer.$theme;
    ...
    function normalizeRef(tsData, ref, path) {
        ...
        if (typeof tsData.doc != "string") {
            ...
            if (tsData.doc.length < 10 || ref.start) {
                ...
                }) == tsData.doc.substr(ref.span.start, ref.span.length), tsData.doc.substr(ref.span
                    .start, ref.span.length) + ' !=! ' + session.getTextRange({
                    start,
                    end
                }));
    ...
    References.prototype.createItem = function(index) {
        ...
        var noHeader = this.items[index - 1] && this.items[index - 1].file == data.file;
        var element = document.createElement('li');
        element.className = "references-item mb-10 " + (noHeader ? "" : "mt-5");
        element.tabIndex = 0;
        var parts = noHeader ? [] : ["<i class='tooltip-filename", data.file.length > 18 ? " clipper" : "",
            "'>", data.file,
            "</i></br><span>"
        ];
        if (data.start) {
            parts.push("<i>Line ", data.start ? data.start.row + 1 : "", ", column ", data.start ? data
                .start
                .column : "", '</i>');
        } else if (data.span) {
