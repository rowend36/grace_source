        ...
        for (var i = 0; i < RERUNS; i++) {
            ...
            it('should resolve sequential changes', function (done) {
                ...
                a.onEdit = function (changes, isLocal) {
                    ...
                    try {
                        expect(this.getStore()).to.deep.have.members(
                            this._current[0][2]
                        );
                        expect(remoteStore.getStore()).to.deep.have.members(
                            this._current[0][2]
                        );
                ...
                a.set(['none']);
                remoteStore.set(['none']);
                a.transport.submitNode = remoteStore.onRemoteNode.bind(
                    remoteStore
                );
                a.transport.connect();
                Utils.asyncForEach(
                    ...
                    function (e, i, n) {
                        if (e[0] == 'set') {
                            ...
                            expect(remoteStore.getStore()).to.have.members(
                                e[2]
                            );
                            n();
                        } else {
            ...
            it('should resolve parallel changes', function (done) {
                ...
                a.onEdit = Utils.debounce(function () {
                    try {
                        expect(a.getStore()).to.deep.have.members(
            ...
            it('should allow no delete', function (done) {
                ...
                var resolve = Utils.debounce(function (item) {
                    try {
                        ...
                        expect(t.getStore()).to.have.members(
                            'abcdef'.split('')
                        );
                        done();
                    } catch (e) {
