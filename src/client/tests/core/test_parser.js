define(function (require, exports, module) {
  var JSONParser = require('grace/ext/json_ext').JSONParser;
  ...
  describe('Parser', function () {
    ...
    before(function () {
      ...
      RESET = {
        state: parser.compile().JSON,
        text: json_sample1k,
      };
    });
    it('should be about as fast as pegjs', function () {
      var time = performance.now();
      for (var i = 0; i < 40; i++) {
        parser.setState({state: RESET.state, text: json_9_4ms});
        parser.walk(new TreeListener());
      }
      var time = (performance.now() - time) / 40;
      expect(time).to.be.lessThan(9.4, time.toFixed(2) + 's > 9.4ms');
    });
    it('should parse 1kb json at least 47-80 times/second', function () {
      parser.setState(RESET);
      parser.listener = {enter: noop, exit: noop, token: noop};
      parser.walk();
      var time = performance.now();
      for (var i = 0; i < 40; i++) {
        parser.setState(RESET);
      ...
      var ops = (1000 * i) / (performance.now() - time);
      expect(ops).to.be.greaterThan(80, ops.toFixed(2) + 'ops/second');
    });
    it('should generate cst', function () {
      var genTree = new TreeListener();
      parser.setState(RESET);
      parser.walk(genTree);
      expect(genTree.getContext().children.length).to.equal(1);
    });
    // Warning: running an uncompiled parser leads to a large amount of slowdown
    // due to deoptimization. It should only be used when the parser is in development.
    it('should parse 1kb json without compilation at least 23-40 times/second', function () {
      ...
      var ops = (1000 * i) / (performance.now() - time);
      expect(ops).to.be.greaterThan(40, ops.toFixed(2) + 'ops/second');
    });
  });
  var json_sample1k =
    '[\r\n' +
