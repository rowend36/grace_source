define(function(require, exports, module) {
    ...
    function ScrollbarMarker(editor) {
        ...
        this.viewport = {
            ...
            update: function(html, self, session, config) {
                self.elt(
                    this.clazz,
                    "height:" + (self.editor.renderer.$size
                        .scrollerHeight * config.scaleY) +
                    "px;" +
                    "width:" + config.width + "px;" +
                    "top:" + (self.editor.renderer
                        .scrollMargin.top + session
                        .getScrollTop()) *
                    config.scaleY + "px;" +
                    "left:" + 0 + "px;"
                );
    ...
    ScrollbarMarker.prototype.scroll = function() {
        var viewport = this.element.getElementsByClassName(
            "marker_square")[0];
        if (viewport) {
            viewport.style.top = (this.editor.renderer.scrollMargin
                    .top + this.session.getScrollTop()) *
                this.config.scaleY + "px";
        }
    };
    ...
    ScrollbarMarker.prototype.gatherMarkers = function() {
        var markers = Object.assign({}, this.session.getMarkers(),
    ...
    ScrollbarMarker.prototype.update = function() {
        ...
        this.cursors.length =
        0; //Marker will remove all the cursor elements
        this.updateCursors();
    };
    ScrollbarMarker.prototype.updateCursors = function() {
        ...
        all.forEach(function(e, i) {
            var f = this.session.documentToScreenPosition(e
                .start);
            var left = Math.min(f.column * this.config
                .characterWidth, VSCROLL_WIDTH - 3);
            var height = Math.max(10, this.editor.renderer
                .getLineHeight(f.row));

            this.cursors[i].style.cssText = "top:" + (this
                    .$getTop(f.row, this.config) - height /
                    2) + "px;left:" + left + "px;" +
                "height: " + height + "px;";
    ...
    ScrollbarMarker.prototype.render = function(changes) {
        ...
        var isScroll = false;
        if (changes & (r.CHANGE_MARKER | r.CHANGE_MARKER_BACK | r
                .CHANGE_MARKER_FRONT)) {
            this.gatherMarkers();
        ...
        var scaleY = Math.min(1, this.element.clientHeight /
            totalHeight);
        var scaleX = (VSCROLL_WIDTH - 5) / Math.min(configOld.width,
            (r.$size.scrollerWidth * 2 + r
                .scrollLeft));
        var config = {
            scaleY: scaleY,
            ...
            firstRowScreen: (-r.scrollMargin.top / configOld
                .lineHeight),
            offset: 0
        };
        if (isScroll) {
            ...
            set: function(val) {
                if (val) {
                    if (!this.$scrollbarMarker) {
                        this.$scrollbarMarker =
                            new ScrollbarMarker(this);
    ...
    function autofadeScrollBars(editor) {
        ...
        ['ontouchstart', 'ontouchmove', 'ontouchend', 'onmousemove']
        .forEach(function(f) {
        ...
        editor.renderer.scrollBarV.element.style.width = VSCROLL_WIDTH +
            "px";
        ...
        editor.on("changeSession", function(s) {
            s.oldSession && s.oldSession.off('changeScrollTop',
                fadeScroll);
            s.session && s.session.on('changeScrollTop',
                fadeScroll);
            fadeScroll();
        });
        editor.renderer.scrollBarV.setVisible(true);
    }
    exports.setupMobileMenu = setupMobileMenu;
    exports.autofadeScrollBars = autofadeScrollBars;
});
