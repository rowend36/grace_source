        ...
        return function (deltas, rev, i) {
            ...
            if (d.isPartialUndo) {
                if (i === deltas.length - 1)
                    filters.push(
                        skipRev(d.$baseRev, d.$baseIndex, d.$endIndex)
                    );
                return false;
            }
        };
    ...
    function _findDeltas(stack, onMatchRev, splitDeltas, prevMatch) {
        ...
        for (var i = start; i > -1; i--) {
            var deltaSet = stack[i],
                ...
                tail;
            var matched = Boolean(
                onMatchRev(deltaSet, rev, deltaSet.length - 1)
            );
            if (splitDeltas || !matched) {
                //selection for split points
                for (var j = deltaSet.length - 2; j > -1; j--) {
                    if (Boolean(onMatchRev(deltaSet, rev, j)) !== matched)
                        break;
                }
                if (j > -1) {
                    if (splitDeltas) {
                        ...
                        if (matched) {
                            ...
                            for (var k = j; k > -1; k--) {
                                if (
                                    Boolean(onMatchRev(deltaSet, rev, k)) ===
                                    matched
                                )
                                    break;
                            }
    ...
    function _invertAndApply(session, deltas, after) {
        ...
        if (
            ...
            (um.$redoStack.length === 0 ||
                um.$redoStackBaseRev !== deltas[0].id)
        ) {
            var d = um.$redoStack;
            um.$redoStack = [];
        ...
        for (var i = deltas.length - 1; i > -1; i--) {
            if (
                deltas[i].action === 'remove' ||
                deltas[i].action === 'insert'
            ) {
                inverse.push({
