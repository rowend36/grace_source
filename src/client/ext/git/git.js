...
define(function(require, exports, module) {
    ...
    var appConfig = require("grace/core/config").Config.registerAll({
        ...
        "gitCorsProxy": Env.isWebView ? undefined : Env
            ._server + "/git",
        "gitSkipGitIgnore": true,
        "gitIgnore": ["logs", "*.log", "npm-debug.log*",
            "yarn-debug.log*", "yarn-error.log*",
            "firebase-debug.log*",
            "firebase-debug.*.log*", ".firebase/", "*.pid",
            "*.seed", "*.pid.lock", "lib-cov",
            "coverage", ".nyc_output",
            ".grunt", "bower_components", ".lock-wscript",
            "build/Release", "node_modules/", ".npm",
            ".eslintcache",
            ".node_repl_history", "*.tgz",
            ".yarn-integrity", ".env**/.*", ".git",
            ".*-git",
            ".git-status", "*~[123]",
        ],
            ...
            if (dir == "***") {
                return Notify.ask(
                    ...
                    function() {
                        ...
                        findRoot(rootDir, fs).then(resolve,
                            reject);
                    },
                    function() {
    ...
    function detectRepo(ev, btn, yes, no) {
        require(["./iso_git", "./cache_fs", "./libs/isomorphic-http",
                "./libs/isomorphic-git-mod.js", "./git_interface",
                "./git_clone", "./git_status", "./git_branch",
                "./git_commit", "./git_log",
                "./git_config", "./git_merge", "grace/ui/itemlist",
                "./git_remote", "./git_fs", "./git_diff",
                "./merge3highlight"
            ],
            function(git) {
                if (!ev) return;
                var dir = ev.rootDir;
                lastEvent = ev;
                findRoot(dir, ev.browser.fileServer).then(function(
                    path) {
                    var fs = lastEvent.browser.fileServer;
                    var GitImpl = fs.$gitImpl || git.Git;
                    prov = new GitImpl(path || dir, join(
                            path || dir, appConfig
                            .gitdir),
                        fs);
                    if (path) {
    ...
    var detectHierarchyRepo = function(ev, btn, yes, no) {
        ...
        ev.browser.expandFolder(ev.filename, function(cb) {
            detectRepo(Object.assign({}, ev, {
                browser: cb,
                rootDir: ev.filepath
            }), ev.browser.getElement(ev.filename)[
                0], yes, no);
        });
    };
    var GitFileMenu = {
        "!update": [function(self, update) {
            update(self, isDirectory(lastEvent.filepath ||
                "") ? null : self[
                "!show-file-status"]);
        }],
        "!show-file-status": {
    ...
    var GitMenu = {
        "currentBranch": {
            ...
            update: function(branch) {
                if (branch != GitMenu.currentBranch
                    .currentBranch) {
                    GitMenu.currentBranch.currentBranch =
                        branch;
        ...
        "repo-actions": {
            ...
            subTree: {
                ...
                "configure-op": Dropdown.defaultLabel(
                    'authentication'),
                "authentication": {
                    icon: "account_circle",
                ...
                "checkout-5": Dropdown.defaultLabel(
                    'Force Checkout'),
                "do-revert-index": {
                    icon: 'warning',
    ...
    GitOverflow.onclick = GitFileOverflow.onclick = NoGitOverflow
        .onclick = function(e, id, span,
            data) {
            prov = prov.cached(id);
            if (data.command) {
    ...
    GitOverflow.ondismiss = GitFileOverflow.ondismiss = NoGitOverflow
    ...
    var GitFileOption = {
        caption: "Git...",
        onclick: function(ev) {
            detectRepo(ev, GitFileOption.anchor,
                GitFileOverflow, NoGitOverflow);
            ev.preventDefault();
        },
        hasChild: true,
        close: false,
    ...
    var GitProjectOption = {
        ...
        sortIndex: 200,
        onclick: function(ev) {
            detectHierarchyRepo(ev, GitProjectOption.anchor,
                GitOverflow, NoGitOverflow);
            ev.stopPropagation();
            ev.preventDefault();
        },
        hasChild: true,
    ...
    var GitOption = {
        ...
        onclick: function(ev) {
            lastEvent = ev;
            detectRepo(ev, GitOption.anchor, GitOverflow,
                NoGitOverflow);
            ev.preventDefault();
            ev.stopPropagation();
        },
    ...
    FileUtils.registerOption("files", ["create"], "git-opts",
        GitOption);
    FileUtils.registerOption("files", [
        "file", "folder"
    ], "git-file-opts", GitFileOption);
    FileUtils.registerOption("project", ["project"], "git-opts",
        GitProjectOption);
