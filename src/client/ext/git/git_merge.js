...
define(function (require, exports, module) {
    ...
    var gitDoConfig = require('./git_config').doConfig;
    var testPlain = GitUtils.testPlain;
    var success = GitUtils.success;
    var gitAddRemote = require('./git_remote').addRemote;
    var gitHighlightMerge3 = require('./merge3highlight');
    ...
    function previewTree(opts, prov) {
        ...
        var el = $(
            Notify.modal({
                ...
                body:
                    (noConflict
                        ? "<h6 class='green-text'>Automatic merge is possible</h6>"
        ...
        function createChangeListItem(item) {
            var card = $(document.createElement('li'));
            var text = item.isDelete
                ? 'Deleted '
                : item.isAdd
                ? 'Added '
                : item.head != opts.ours
                ? 'Updated ' +
                  (item.mode ? 'and changed mode to ' + item.mode : '')
                : item.mode
                ? 'Changed mode to ' + item.mode
        ...
        function reviewConflict(mergeIndex) {
            ...
            if ((doc = Docs.forPath(docPath))) {
                ...
                doc = Docs.get(
                    openDoc(
                        null,
                        new MergeDoc(item.mergedText, ourHref + item.path)
                    )
                );
                //save somewhere else
                doc.fileServer = opts.fs.id;
    ...
    exports.doMerge = function (ev, prov) {
    ...
    function startMerge(ev, prov) {
        ...
        el.on('submit', function () {
            var result = require('grace/ui/forms').Forms.parse(el);
            var ref = result.branchSelect;
            var progress = createProgress(
                'Merging ' + currentBranch + ' with ' + ref
            );
            ...
            function finish(result) {
                if (!result.fastForward) progress.dismiss();
                if (result.sameRef) {
                    Notify.info('Both branches are identical');
                } else if (result.alreadyMerged) {
                    Notify.info(
                        'Target branch has already merged into this branch'
                    );
    ...
    /*refs and remotes*/
    exports.doPush = function (ev, prov) {
        exports.doPull(ev, prov, true);
    };
    exports.doPull = function (ev, prov, isPush) {
            ...
            if (!b.length) {
                gitAddRemote(ev, prov, function () {
                    prov.listRemotes().then(function (b) {
                        ...
                        el.find('select').val(
                            appConfig.defaultRemote || b[0].remote
                        );
                    });
                });
            }
        ...
        el.find('select').on('change', function () {
            configure('defaultRemote', el.find('select').val(), 'git', true);
        });
            ...
            prov[method]({
                ...
                onAuthSuccess: function () {
                    ...
                    Notify.ask('Save login details?', function () {
                        configure('gitUsername', config.username, 'git', true);
                        configure('gitPassword', config.password, 'git', true);
                    });
                },
                onProgress: progress.update,
                onAuthFailure: function () {
                    gitDoConfig(ev, prov, retry);
                },
            }).then(
                function () {
                    progress.dismiss();
        ...
        el.on('submit', function (e) {
            ...
            if (t) configure('defaultRemoteBranch', t, 'git', true);
    ...
    function restartMerge(tree, prov) {
        tree.href =
            'git-merge:/' +
            tree.ourOid.substring(0, 5) +
            '.' +
            tree.theirOid.substring(0, 5) +
            '/';
        tree.dir = prov.dir;
        Docs.forEach(function (doc) {
            if (
                doc.constructor == MergeDoc &&
                doc.getPath().startsWith(tree.href)
            ) {
                doc.mergeTree = tree;
                doc.gitdir = tree.gitdir;
            }
        });
        //todo save merge
