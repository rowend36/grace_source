define(function (require, exports, module) {
    ...
    SharedStore.prototype.get = function () {
        if (!this._hasRead) {
            this.stack.reset(
                this.transport.getInitialValue() ||
                    this.createNode(this._format.NULL)
            );
    ...
    SharedStore.prototype.syncStore = function (wasFastForward, isLocal) {
        ...
        if (this._hasRead) {
            var changes = wasFastForward
                ? n.patches
                : this.getPatches(m, n.value);
            if (changes.length) {
    ...
    SharedStore.prototype.onRemoteNode = function (node) {
        ...
        if (insertPoint > -1) {
            ...
            if (!node.isMerge)
                console.warn(
                    this.id,
                    ' discarded stale data:(' + node.version + ')'
                );
        }
    };
    ...
    SharedStore.prototype._isSameMerge = function (winner, node2) {
        if (
            ...
            winner.versions = (node2.versions || []).concat(
                winner.versions || []
            );
 ...
 (reverserNode)ba abc  \
    ...
    SharedStore.prototype._mergeRebase = function (loser, base) {
        ...
        if (this._format.transform) {
            ...
            loser.reverserNode.patches = this.getPatches(
                loser.value,
                base.value
            );
        } else {
        ...
        var result = this.stack.walk(
            ...
            function onEnter(ctx, i, parentCtx) {
                ...
                var inBadMerge = parentCtx
                    ? (node.isMerge && node.base === conflictBase) ||
                      parentCtx.inBadMerge
                    : false;
