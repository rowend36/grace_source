    ...
    TsClient.prototype = Object.assign(Object.create(BaseClient.prototype), {
        ...
        $unfixName: function (name) {
            ...
            if (name.startsWith(TEMP_DIR))
                return (
                    'temp:/' + name.slice(TEMP_DIR.length, name.lastIndexOf('.'))
                );
            else {
                ...
                refs: data.map(function (e) {
                    ...
                    return {
                        ...
                        start: session && toAceLoc(session, e.textSpan.start),
                            ...
                            toAceLoc(
                                session,
                                e.textSpan.start + e.textSpan.length
                            ),
        ...
        genArgHintHtml: function (args, activeIndex) {
            ...
            for (var j = 0; j < doc.parameters.length; j++) {
                for (
                    var m = 0;
                    m < doc.parameters[j].displayParts.length;
                    m++
                ) {
                    html.push(
                        createToken(
        ...
        resolveDependencies: function (dir, missing, cb) {
            var ts = this;
            if (cb) ts.once('loaded-deps', cb);
            if (ts.$loadingDeps) return;
            ts.$loadingDeps = true;
            require('./add_dependencies').addDependencies(
                this,
                dir,
                missing,
                function (err, didAdd) {
                    ts.$loadingDeps = false;
                    //In case we ignored any new requests
                    if (didAdd) ts.updateAnnotations(getEditor());
                    ts.trigger('loaded-deps');
                }
            );
        },
                ...
                function (n, e, data) {
                    if (data) {
                        data.refs.forEach(function (e) {
                            ...
                            e.end =
                                doc &&
                                toAceLoc(doc, e.span.start + e.span.length);
                        });
                    }
                    return n(e, data);
                },
    ...
    var BasicTransport = function (onRequestFile) {
        ...
        this.postMessage = Depend.after(
            ...
            function (data, cb) {
                ...
                try {
                    res = (
                        instance[data.type] || instance.getLSP()[data.type]
                    ).apply(instance, data.args);
                } catch (e) {
                    error = e;
                    debug.error(error);
                }
