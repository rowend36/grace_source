define(function (require, exports, module) {
    ...
    BaseClient.prototype = {
        ...
        loadFiles: function (data, cb) {
            ...
            Utils.asyncForEach(
                data.refs,
                function (ref, i, next) {
                    if (data.files[ref.file] || ts.docs[ref.file])
                        return next();
                    ...
                    ts.options.switchToDoc(
                        ...
                        function (err, editor) {
                            if (!editor)
                                ...
                                data.name != getDoc(ts, editor.session).name
                            )
                                cancel(
        ...
        addDoc: function (name, session_or_text, preferSession) {
            ...
            var session =
                typeof session_or_text == 'string' ? null : session_or_text;
            ...
            if (!data.fragOnly)
                this.sendDoc(
                    session ? data : Object.assign({text: text}, data)
                );
    ...
    function getCallPos(editor, pos) {
        ...
        if (
            ...
            end.column + token.value.length > pos.column
        ) {
            token = {
                type: token.type,
