    ...
    TagsClient.prototype = {
        ...
        loadTags: function (
            ...
            if (
                noOverwrite &&
                files[filename] &&
                files[filename]['!mode'] == mode
            )
                return;
            if (
                !files[filename] &&
                !this.isOptionEnabled('loadFileTypes', mode)
            )
                return;
            ...
            (doc ? tagger.findAllDoc : tagger.findAll || tagger)(
                ...
                function (tags) {
                    ...
                    this.tags.loadParsedTags(
                        filename,
                        {
                            '!scopes': scopeSolvers[mode]
        ...
        requestDefinition: function (editor, cb) {
            ...
            if (!args)
                return this.ui.showInfo(editor, 'No name found at position');
        ...
        getQueryArgs: function (editor, cursor, name) {
            ...
            if (!name) {
                ...
                if (!name || !/\w/.test(name.value))
                    name = editor.session.getTokenAt(
                        cursor.row,
                        cursor.column + 1
                    );
