...
or actual source files( will be parsed.',
            ...
            autoUpdateTags: {
                type: 'boolean|array<mode>',
                doc:
                    'Whether tags should be parsed from the current file automatically on load and whenever changes are made. Uses resource context.',
            },
        },
        'intellisense.tags'
    );
    ...
    function isOptionMatching(_option, value, path) {
        var m =
            memo[_option] || (memo[_option] = globToRegex(appConfig[_option]));
        return m.test(value);
    }
