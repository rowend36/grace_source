define(function (require, exports, module) {
  ...
  */
    ...
    function disableProvider(editor, p) {
        if (Utils.removeFrom(editor.$activeProviders, p) > -1)
            Utils.removeFrom(editor.completers, editor[p.name]);
    }
    function addCompleter(editor, e) {
        if (editor.completers.indexOf(e) < 0) {
            editor.completers.push(e);
        }
    }
    function updateCompleters(editor) {
        ...
        var toEnable =
            editor.completers && editor.$enableIntelligentAutocompletion
        ...
        toEnable.forEach(function (provider) {
            ...
            if (active.indexOf(provider) > -1) return;
    ...
    EditorSettings.addOption('enableIntelligentAutocompletion', {
        set: function (val) {
            if (val) {
                if (this.$inDotCompletion === undefined)
                    this.$inDotCompletion = false;
