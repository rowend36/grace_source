define(function (require, exports, module) {
    ...
    */
    ...
    var appConfig = require("grace/core/config").Config.registerAll({}, "toolbars");
    var RESET_CHAR = "\n\nmod-";
    var TAIL = "\n\n";
    ...
    function setSelection(immediate) {
        ...
        modKeyInput.selectionStart = modKeyInput.selectionEnd = RESET_CHAR.length;
    ...
    function openModKey() {
        ...
        var keepFocus =
            shifted &&
            (closed ? FocusManager.activeElement : host) != editor.textInput.getElement();
        if (keepFocus) {
            if (closed) return;
            return closeModKey();
        }
    ...
    function closeModKey(forced) {
        if (closed) return;
        closed = true;
        if (host != editor.textInput.getElement()) $(host).removeClass("mod-key-enabled");
        if (forced) {
            pressed = {};
            held = {};
            triggerUpdate();
    ...
    var clipboardHandler = new (ace.require("ace/keyboard/hash_handler").HashHandler)();
    clipboardHandler.bindKeys(editorHandler);
    require("grace/core/file_utils").FileUtils.on("clear-temp", function () {
        closeModKey(true);
    });
