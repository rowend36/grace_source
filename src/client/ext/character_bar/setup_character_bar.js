define(function(require, exports, module) {
    ...
    var appConfig = require("grace/core/config").Config.registerAll({
            ...
            enableModKeys: !Env.isHardwareKeyboard &&
                "shift,ctrl,alt",
...
--W-- represents the word before the cursor - Note: using it will also remove the word\n",
    ...
    configEvents.on("toolbars", function(e) {
        switch (e.config) {
            case "characterBarChars":
                ...
                break;
            default:
                postRebuild = postRebuild || Utils.delay(
                    rebuild);
                postRebuild();
        }
    });
    ...
    Mods.setChangeListener(function update(key, ev) {
        ...
        if (key) {
            ...
            var isMod = key == "ctrl" || key == "alt" || key ==
                "shift";
            var name = target.className.split(" ");
            setCls(name, "mod-key-held", ev.held[key]);
            setCls(name, "mod-key-active", !isMod || (ev
                .pressed[key] && !ev.held[key]));
            setCls(name, "color-inactive", isMod && !(ev.held[
                key] || ev.pressed[key]));
    ...
    function createToolsView(tools, scrollPoints) {
        ...
        for (var b = 0; b < tools.length; b++) {
            ...
            var text = data.caption || "<i class='material-icons'>" +
                icon + "</i>";
    ...
    function createCharactersView(chars) {
        var bar = document.createElement("div");
        for (var o in chars) {
            var i = require("grace/core/utils").Utils.htmlEncode(chars[
                o]).replace(/'/g, "\\'");
            ...
            $(bar).append(el);
            if (t) {
                var displayLength = caption.replace(/&\w+;/g, "-")
                    .length;
                $(bar)
                    .children()
                    .last()
    ...
    function handleCharClick(e) {
        ...
        var target = FocusManager.activeElement || editor.textInput
            .getElement();
    ...
    function rebuild() {
        ...
        if (appConfig.enableModKeys !== true) {
        ...
        var event_mousedown = "ontouchstart" in window ? "touchstart" :
            "mousedown";
        var event_mouseup = "ontouchend" in window ? "touchend" :
            "mouseup";
        var event_mousemove = "ontouchmove" in window ? "touchmove" :
            "mousemove";

        var repeat;
        if (appConfig.toolbarPosition)
            toolbar = $(createToolsView(tools, [
                ["esc", "shift", "ctrl", "a-left"], "paste"
            ]));
        if (modtools) {
            metabar = $(createToolsView(modtools));
        }
        Mods.setClipboard(require(
            "grace/ext/clipboard/enhanced_clipboard").clipboard);

        if (toolbar || metabar) {
            repeat = RepeatDetector(appConfig,function(command) {
        ...
        if (appConfig.showCharacterBar) {
            charbar = $(createCharactersView(appConfig
                .characterBarChars));
        ...
        if (bottomBar && bottomEls.length > 0 && appConfig
            .showCharacterBarToggle) {
        ...
        if (bottomBar && bottomEls.length < 1) {
            ...
            if (!swipe) {
                swipe = SwipeDetector(bottomBar, function() {
                    ...
                    bottomView.layout_height = bottomBar
                        .height();
                    bottomView.parent.render();
                    if (floatingToggle) floatingToggle.css(
                        "bottom",
                        bottomView.layout_height);
                });
                ...
                if (!pageToggle) bottomEls[0].css("padding-left",
                    "20px");
            } else {
                bottomView.show();
    ...
    var updateRecordingIcon = Utils.delay(function() {
        ...
        } else $("#togglerecording", toolbar).removeClass(
            "blink");
    }, 100);
    var trackStatus = function(e) {
        (e.editor || e).on("changeStatus",
            updateRecordingIcon);
    };
