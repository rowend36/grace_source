define(function(require,exports,module){
    ...
    var domQueryAll = $;
    var domQuery = function(sel, ctx) {
        return $(sel, ctx)[0];
    };
    var domAttr = function(el, attr, val) {
        return val!==undefined?$(el).attr(attr, val):$(el).attr(attr);
    };
    ...
    DragTabs.prototype._setDraggable = function() {
        ...
        forEach(allTabs, function(i,tabNode) {
    ...
    DragTabs.prototype._moveTab = function(event) {
        ...
        var target = $(event.target).closest(this.options.selectors.tab)[0]||event.target,
            dragTab = context.dragTab,

            newIndex;

        if (!tabContainer.contains(target) ||
    ...
    DragList.prototype._moveTab = function(event) {
        ...
        // would not immediately swap back (when dragging a small element
        // over a bigger one)
        var delta = targetHeight - dragTab.offsetHeight;
        ...
        if (delta > 0) {
            ...
            // we'd see a
            if (offset > event.offsetY || (targetHeight - offset) < event.offsetY) {
                return cancelEvent(event);
            }
        }
