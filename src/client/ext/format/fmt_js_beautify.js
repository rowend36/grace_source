    ...
    require('grace/core/config').Config.registerInfo(
        {
            ...
            wrap_attributes_indent_size: {
                values: [
                    ['null', 'Sets this to the same value as indent_size'],
                    '<number>',
                ],
                doc:
                    'Indent wrapped attributes to after N characters (default:[null]) (ignored if wrap_attributes is "aligned")',
            },
            inline: {
                values: [
                    ['null', 'Use the default list of html inline elements'],
                    ['<array<string>>', 'Custom list'],
                ],
        ...
        return function (val, opts, cb, flags) {
            require(files, function (mod) {
                ...
                if (typeof val === 'object') {
                    val =
                        flags && flags.isPartialFormat
                            ? val.getTextRange(flags.range)
                            : val.getValue();
                }
                ...
                try {
                    ...
                    err = e;
                    if (!(flags && flags.silent)) Notify.error(e);
                }
                cb(
                    val,
                    null,
                    //Jshint will preserve baseindent if range contains indent
                    err ? false : !(flags && flags.rangeContainsIndent)
                );
            });
        };
    }
