  ...
  */
    ...
    return function(val, opts, cb, data) {
      require(deps, function(prettier, plugin) {
        ...
        try {
          ...
          err = e;
          Notify.error("<pre>" + Utils.htmlEncode(e.toString()) +
            "</pre>");
        }
        cb(val, null, !err);
      });
  ...
  function beautifier(file, name) {
    return function(val, opts, cb, data) {
      ...
      var err = null;
      try {
        val = require(file)[name](val, opts);
      } catch (e) {
        err = e;
        Notify.error(e);
      }
  ...
  var dependencyMap = {
    "prettier_js": [
      ["./libs/prettier", "./libs/parser-babel"], pretty([
        "./libs/prettier", "./libs/parser-babel"
      ]),
    ],
    "prettier_css": [
      ["./libs/prettier", "./libs/parser-postcss"], pretty([
        "./libs/prettier", "./libs/parser-postcss"
      ])

    ],
    "prettier_md": [
      ["./libs/prettier", "./libs/parser-markdown"], pretty([
        "./libs/prettier", "./libs/parser-markdown"
      ])

    ],
    "prettier_php": [
      ["./libs/prettier", "./libs/parser-php"], pretty([
        "./libs/prettier", "./libs/parser-php"
      ])

    ],
    "js_beautify": [
      ["./libs/beautify.min"],
      beautifier("./libs/beautify.min", 'js_beautify')
    ],
    "html_beautify": [
      [
        "./libs/beautify.min",
        "./libs/beautify-css.min",
        "./libs/beautify-html.min"
      ], beautifier("./libs/beautify-html.min", 'html_beautify')
    ],
    "css_beautify": [
      ["./libs/beautify-css.min"],
      beautifier("./libs/beautify-css.min", 'css_beautify')
    ]
  };
  ...
  function getCurrentMode(editor, allowNestedMode) {
    ...
    if (allowNestedMode) {
      var temp = getNestedModeforRow(editor, editor.getCursorPosition()
        .row);
      if (temp !== '') return temp;
    }
    return scope;
  ...
  function createBeautifier(item, mode) {
    return (function() {
      var args = arguments;
      //todo remove all non-static require
      require(item[0], function() {
        item[1].apply({
          mode: mode
        }, args);
      });
  ...
  function getBeautifier(mode) {
    for (var j in dependencyMap) {
      var modes = Utils.parseList(appConfig[j + "_modes"]);
      if (modes.indexOf(mode) > -1) {
        return createBeautifier(dependencyMap[j], mode);
      }
    }
    return null;
  }
  ...
  function setValue(session, value, sel, originalRangeStart, range,
    formatAll, unselect) {
    try {
      var deltas;
      if (typeof value == 'object') {
        ...
        if (!value[0] || value[0].start) {
          deltas = value;
        } else deltas = Docs.diffToAceDeltas(value, formatAll ? 0 :
          originalRangeStart.row, formatAll ? 0 :
          originalRangeStart
          .column);
      } else {
        if (formatAll) {
        ...
        deltas = Docs.generateDiff(formatAll ? session.getValue() : session
          .getTextRange(range), value,
          formatAll ? 0 :
          originalRangeStart.row, formatAll ? 0 : originalRangeStart
          .column);
      }
      var end = range.start;
  ...
  function beautify(editor, unselect, removeBlanks, overrideOptions) {
    ...
    if (range.start.row === range.end.row && range.start.column === range
      .end.column) {
      range.start.row = 0;
    ...
    if (removeBlanks) {
      ...
      value = value.replace(/ +$|(\S ) +(\S)/gm, "$1$2").replace(
        /((?:\r\n|\n|\r){3})(?:\r\n|\n|\r)*/g, "$1");
      setValue(session, value, sel, originalRangeStart, range, formatAll,
        unselect);
      return true;
    }
    ...
    if (detectedMode == "html" || detectedMode == "php" || detectedMode ==
      "svg") {
      if (!
        formatAll
    ...
    if (doc && doc.flags && doc.flags.formatter) {
      beautifier = createBeautifier(dependencyMap[doc.flags.formatter],
        null);
      options.options = doc.flags.formatOptions;
    } else beautifier = getBeautifier(detectedMode);
    if (doc && doc.flags && doc.flags.formatOptions)
      Object.assign(options.options, doc.flags.formatOptions);
    if (!beautifier) {
    ...
    beautifier(value, options, function(value, pos, shouldPad) {
      ...
      setValue(session, value, sel, pos || originalRangeStart, range,
        formatAll, unselect);
    }, {
  ...
  function onAfterExec_Beautify(e) {
    ...
    if (e.command.name === "insertstring" && e.args === "}") {
      ...
      if (tok) {
        if (tok.type.indexOf('paren') > -1 && tok.type.toString().indexOf(
            'comment') === -1) {
  ...
  exports.getBeautifier = getBeautifier;
}); /*_EndDefine*/