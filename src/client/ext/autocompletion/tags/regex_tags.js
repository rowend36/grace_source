define(function(require,exports,module) {
    ...
    */
    ...
    require("grace/core/utils").Utils.inherits(RegexTags, require("./async_tags").AsyncTags);
    ...
    var S = {
        ...
        bracketRange: function(s, e) {
            ...
            s = "\\" + s;
            e = "\\" + e;
            return S.s(s + "(?:[^" + e + s + "]{1,100}(?![^" + e + s + "])|#BRACKET){0,5}" + e)
                .recurse("#BRACKET", 2);
        },
        string: function(s) {
            if (this != S) throw new Error('Static method');
        ...
        test: function(str, willPass) {
            ...
            if (!res && willPass !== null) {
                ...
                if (willPass === null) {
                    ...
                    var isCorrect = res[0] == (typeof willPass == "string" ? willPass : str);
                    if (willPass === false ? isCorrect : !isCorrect) {
                        debug.log(res);
                        debug.log(toChars(res[0]), toChars(typeof willPass == "string" ?
                            willPass : str));
    ...
    /*Basic variable name*/
    S.ident = S.s("[a-zA-Z_\\$][\\-_\\$a-zA-Z0-9]*\\b").test("abcd").test("9abc", "abc").test("}", null).o();
    S.lineStart = S.s("^").add(S.s().sp()).o();
    //adds a group
    S.comments = S.s(/\/\*((?:[^\*]|\*[^\/]){0,150})\*\//).o();
    S.bracketRange('(', ')').test("(()))", "(())");
    //Javascript
    S.jsStrictStart = S.comments( /*1*/ ).sp().sp3().maybe().add(S.lineStart().group(2)).test("/*hello*/\n   ")
        .o();
    ...
    S.jsPropChain = S.ident(S.s().sp2().t("\\.")).sp2().wrap().test("pako.").test("op.op", false).o();
    ...
    S.jsArguments = S.s("\\(").sp().add(/(?!\d|[^\)]*function\b)/).sp2()
        .add(S.s("[^\\)\\n\\r\\(]*(?:\\n[^\\(\\)\\n]*){0,3}\\)").maybe())
        .test("(hi)") //true
        .test("(function(kop){})", false)
        .o(); //signature

    S.jsVar = S.oneOf(["var", "const", "let", "readOnly"]).t("\\b").or(S.jsPropChain().t("+")).group(3).maybe()
        .sp()
        .add(
    ...
    S.c_ident =
        //c pointers
        S.s("\\*").sp().maybe(true).add(
            S.ident().add(
                S.s("[:\\.]:?").add(S.ident()).star()),
            //c templates
            S.s().sp().add(S.bracketRange("<", ">")).maybe(),
            //arrays
            S.s().sp().t("\\[").sp().add(S.s(/\d+/).sp().maybe()).t("\\]").star()
        )
        .wrap()
    ...
    S.c_start(" *p").test("p").test("\np", "p").test("{\n  p", "  p")
        .test("  p");
    S.modifier = S.oneOf(["public", "synchronized",
            "inline", "protected", "private",
            "abstract", "final",
            "static", "virtual",
            "default", "strictfp"
        ])
    ...
    S.c_args = S.s().sp2().t("\\(")
        ...
        ).test("abcd(a b cd[ ], bcd ef{)", null).test("abcd(a b cd[ ], bcd ef)hekko", "(a b cd[ ], bcd ef)")
        .o();

    S.splitDeclList = S.s().sp2().add(S.safeVal().maybe()).add(S.oneOf([",", "$"])).create();

    //C based languages keywords
    var sharedKeywords = [
        "if", "while", "for", "switch",
        "catch", "public", "private", "abstract",
        "final", "static", "virtual", "struct",
        "union", "default", "class", "enum",
        "interface", "new", "return", "throw",
        "throws", "else", "case"
    ];
    var Regexps = {
        "jsFuncDeclaration": {
            //function/method declaration and calls
            re: S.jsStrictStart().maybe().add(
        ...
        "jsVarFunction": {
            ...
            re: S.jsStrictStart().maybe().add(S.jsVar().sp2().maybe()
                    .test("")
                    .add(
                        /((?:async\b\s*)?function)?\s*(\w+)?/).sp2()
                    .test("async function hello")
                    .add(S.jsArguments().group()).sp2()
        ...
        "jsVarDeclList": {
            paramRe: S.s().group(1).add(S.c_ident().group(2)).sp().add(S.safeVal().group(3).maybe())
                .create(),
            re: S.jsLaxStart().maybe()
                .add(S.oneOf(['var', 'const', 'let', 'readOnly']).group(3).sp1())
                .add(
                    S.c_ident().add(
                        S.safeVal().maybe()
                        .add(
                    ...
                    .test("a").test("a=b,p").test("a={},c").test("a=v,a={{}", "a=v,a")
                    .test("a=v", false) /*Don't handle singleVariable assignments*/
                    .group(4)).sp().add(/[;$\n\r\{]/)
            ...
            handle: function(pos, text, comments, indent, type, list) {
                ...
                return declList(names, pos + text.indexOf(list), type, this.paramRe);
            }
        },
        //finds properties and assignment declarations
        "jsVarSingleDecl": { //variables
            re: S.jsLaxStart().maybe().add(S.jsVar( /*adds groups 3-5*/ ))
                .test("jsVarSingleDecl:")
                .add(
                    S.s(/(?:new *(\w+)\b)/).maybe() /*7*/ .add(
                        S.s(/.{0,50}/).group(8),
                        S.s(/\,\{|\;|$/).wrap()).group(6)
                )
                .create(),
            handle: function(index, text, comments, indent, propChain, isQuoted, name, rightHandSide, type,
                value) {
                type = type || (rightHandSide[0] == "'" || rightHandSide[0] == '"' ? "string" : "");
                var res = [];
                var isProperty = propChain && propChain[propChain.length - 1] == ".";
                //filter out functions
                if ((type || !(value.startsWith("function") || /^\(.*=\s*>(?:\{|$)/.test(value)))) {
                    //figure out if it's a declaration
                    if (!(propChain || isQuoted || (text.indexOf(":") < text.indexOf(rightHandSide)))) {
                        //just an assignment
                        res = [name];
                    } else {
                        res = [
                            variable(index + text.indexOf(name), name, type ||
                                propChain, isProperty ? "property" : 'var',
                                join(propChain,
                                    '\n', comments))
                        ];
                    }
        ...
        "cppFunc": {
            ...
            paramRe: S.c_ident().group(1).sp1().add(S.c_ident().group(2)).add(S.s().sp().add(
                S.comments( /*group 3*/ ).maybe())).create(),
            re: S.s().add(
                    S.decorator().sp3()
                    .test("@Override maybe\n")
                    .maybe(),
            ...
            handle: function(pos, all, comments, indent, modifier, ret, name, args) {
                ...
                return argument(args || "", pos, all, this.paramRe).concat([func(pos + all.indexOf(
                        name), name,
                    args, join(join("modifiers:", " ", modifier), '\n', comments), ret)]);
            }
        },
        "cppVar": {
            keywords: sharedKeywords,
            ...
            pointer: /^\s*\*\s*/,
            re: S.c_decl( /*groups 1 and 2*/ ).add(
                    S.c_ident().group(3).sp1() //type
                )
                .add(S.c_ident().add(S.safeVal().maybe().add(
                    S.s().sp2().t(",").sp2().add(
                        S.c_ident()).add(S.safeVal().maybe()).star()
                )).group(4)).sp().add(/[;\n\{\r]/)
                .create(),
        ...
        "cppClass": {
            re: S.s("^").sp().group(1)
                .add(
                    S.oneOf(["public", "private", "abstract", "final", "static",
                        "struct", "union", "virtual", "default", "protected"
                    ]).test("pako", null).sp1().star().group(2).add(
                        S.oneOf(["[Cc]lass", "[Ee]num", "[Ii]nterface", "struct", "union"])).test("class")
                ).sp1()
                .add(S.ident().group(3)).sp2()
                .add(S.s("[^\\{]").star().group(4).test(" extends Pako {", " extends Pako ")).sp()
                .t("(?=\\{)").create(),
        ...
        "pyFunc": {
            re: S.s().add(
                ...
                S.c_args().group(4),
                S.s().sp2(),
                S.s(/\- ?\>[ \t]*(\S+)/).group(5).sp().maybe(),
                S.s(":")
            ),
            handle: function(index, text, doc, indent, name, args, ret) {
                return argument(args || "", index, text).concat([func(index + text.indexOf(
                        name),
                    name, args, doc, ret)]);
            }
        },
    ...
    S.jsVar = S.jsFuncStart = S.jsFuncDeclStart = S.jsFuncBase = S =
        null;
    ...
    Object.assign(require("./completer").TagFinders, {
        'javascript': JS_TAGS,
        'jsx': JS_TAGS,
        'json': JS_TAGS,
        'java': CPP_TAGS,
    ...
    // require("./completer").TagFinders.tsStyle = JS_TAGS;
    // require("./completer").TagFinders.jsStyle = JS_TAGS;
    // require("./completer").TagFinders.pythonStyle = PYTHON_TAGS;
    // require("./completer").TagFinders.cStyle = CPP_TAGS;
}); /*_EndDefine*/