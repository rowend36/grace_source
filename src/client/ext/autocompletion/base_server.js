define(function(require, exports, module) {
    ...
    var TokenIterator = require(
        "grace/ext/ace_loader!ace/token_iterator").TokenIterator;
    var getEditor = require("grace/setup/setup_editors").getEditor;
    var rebind = Utils.debounce(function() {
        var editor = require("grace/setup/setup_editors")
            .getEditor();
    ...
    require("grace/core/config").ConfigEvents.on(
        ...
        function(ev) {
            ...
            if (IsKey.invalid(key)) {
                ...
                require("grace/ui/notify").Notify.info(
                    'Bad Key string for ' + ev.config);
                ev.preventDefault();
            }
        });
    var debug = console;
    ...
    function BaseServer(options, iconClass) {
        ...
        if (!this.options.hasOwnProperty('switchToDoc')) this.options
    ...
    BaseServer.prototype = {
        ...
        jumpToDef: function(editor) {
            ...
            if (ts.currentJumpList) {
                ...
                if (lastJump && Math.abs(lastJump.compareRange(
                        currentPos)) < 2) {
                    var index = ts.currentJumpList.indexOf(
                        lastJump.data) + 1;
                    if (index > 0) {
                        if (index == ts.currentJumpList
                            .length) {
                            index =
                            0; //continue reusing jumpList in case sorting has changed
                        }
                        ts.markPos(editor);
                        ts.goto(editor, ts.currentJumpList[
                            index]);
                        return;
                    }
                }
        ...
        trackChange: function(change, session) {
            if (change.action === "remove" || change.action ==
                "insert") {
                var data = getDoc(this, session);
                if (data.changes) {
                    data.changes.push(change);
                    if (data.changes.length > session
                        .getLength()) {
            ...
            if (argHints && argHints.doc == session && cmpPos(
                    argHints.start, change.end) <= 0) {
        ...
        addDoc: function(name, session_or_text, preferSession) {
            ...
            if (this.docs[name]) {
                if (this.docs[name].doc.getLine && !
                    session_or_text.getLine) {
                    if (preferSession !== false) return this
                        .docs[name];
                }
                this.closeDoc(name);
            }
            if (session_or_text.constructor.name !== 'String') {
                if (session_or_text.getValue().length > this
                    .maxSize) {
                    data.fragOnly = true;
                } else {
                    session_or_text.on("change", this
                        .trackChange);
                }
                session_or_text.on("destroy", this
                    .$onDestroySession);
            } else if (session_or_text.length > this.maxSize)
                data.doc = "";
        ...
        triggerUpdateAnnotations: function() {
            ...
            this.updateAnnotations(editor, function(res) {
                editor.$errorsProvider.update(docId,
                    res);
            });
        },
        refreshDoc: function(editor, full) {
            if (full) {
                ...
                this.ui.showInfo(editor,
                    'Fully refreshed (reloaded current doc and all refs)'
                    );
                return;
            }
            var doc = getDoc(this, editor.session);
            //for now we handle large docs by not loading them
            if (doc.fragOnly) return this.ui.showError(editor,
                'Document Too Large');
            this.sendDoc(doc);
        ...
        goto: function(editor, data, cb, keepTips, ignoreClosed) {
            ...
            if (ignoreClosed && (!localDoc || typeof(localDoc
                    .doc) == 'string')) return cb && cb();
            moveTo(this, editor, doc, (localDoc && localDoc.doc
                .getLine) ? localDoc : {
                name: data.file,
                doc: localDoc && localDoc
                    .doc //optional if we want to use document res
            }, data, keepTips);
        ...
        closeDoc: function(name) {
            name = this.normalizeName(name);
            if (this.docs[name].doc.constructor.name !=
                'String') {
                this.docs[name].doc.off("change", this
                    .trackChange);
                this.docs[name].doc.off("destroy", this
                    .$onDestroySession);
        ...
        hasDoc: function(name_or_session) {
            if (typeof name_or_session == 'string') {
                return this.docs[this.normalizeName(
                    name_or_session)];
        ...
        requestRename: function(editor, newName, cb, refs) {
            ...
            var load = function() {
                ...
                if (typeof doc.doc == 'string') {
                    return ts.options.switchToDoc(file,
                        null, null, doc.doc,
                        function() {
                            if (doc.name != getDoc(ts,
                                    editor.session)
                                .name) return cb(
        ...
        showType: function(editor, pos, calledFromCursorActivity) {
            if (calledFromCursorActivity) {
                if (editor.completer && editor.completer
                    .popup && editor.completer.popup.isOpen)
                    return;
            }
            var ts = this;
            ...
            var cb = function(error, data) {
                ...
                if (!data) return;
                var tip = ts.genInfoHtml(data, false,
                    calledFromCursorActivity);
                ts.ui.closeAllTips();
                ts.ui.makeTooltip(null, null, tip, editor,
                    true);
            };
            try {
                this.requestType(editor, pos, cb,
                    calledFromCursorActivity);
            } catch (e) {
                debug.error(e);
            }
        ...
        executeRename: function(editor, newName, r) {
            var self = this;
            this.requestRename(editor, newName, function(error,
                changes) {
                if (error) return self.ui.showError(
                    editor, error);
                applyChanges(self, changes, newName,
                    function(result) {
                        //ts.ui.refsTooltip(editor,result)
                        var infoMsg = "Replaced " +
                            result.replaced +
                            " references sucessfully";
                        var errorMsg = "";
                        if (result.replaced != r
                            .refs.length) {
                            errorMsg =
                        ...
                        if (errorMsg !== "") {
                            ...
                            self.ui.showInfo(editor,
                                infoMsg);
                        }
                    });
            }, r);
        },
        genInfoHtml: function(
            doc /*, fromCompletion, fromCursorActivity*/ ) {
            ...
            for (var j in doc.documentation) {
                html.push("<div>" + doc.documentation[j].text +
                    "</div>");
            }
            return html.join("");
        },
        genArgHintHtml: function(args, argpos) {
            ...
            for (var i in doc.prefixDisplayParts) {
                html.push(createToken(doc.prefixDisplayParts[
                    i]));
            }
            for (var j = 0; j < doc.parameters.length; j++) {
                for (var m = 0; m < doc.parameters[j]
                    .displayParts.length; m++) {
                    html.push(createToken(doc.parameters[j]
                        .displayParts[m], j == argpos ?
                        "active" :
                        null));
                }
                if (j < doc.parameters.length - 1)
                    for (var k in doc.separatorDisplayParts) {
                        html.push(createToken(doc
                            .separatorDisplayParts[k]));
                    }
            }
            for (var l in doc.suffixDisplayParts) {
                html.push(createToken(doc.suffixDisplayParts[
                    l]));
            }
            /*
    ...
    function showHoverTooltip(ts, editor) {
        ...
        if (ts.functionHintTrigger) {
            ...
            if (!b || trigger.indexOf(b) < 0) {
                ...
                b = token.replace(/\s+$/, "");
                if (!b || trigger.indexOf(b.substring(b.length - 1)) <
                    0)
                    return ts.ui.closeArgHints(ts);
            }
        }
        ...
        if (cache && cache.doc == editor.session && cmpPos(start, cache
                .start) === 0 && cache.name == callPos
            .name) {
        ...
        function inner() {
            ...
            ts.requestArgHints(editor, start, function(cache) {
                ...
                if (debugCompletions) debug.timeEnd(
                    'get definition');
            });
        }
    }
    ...
    function getDoc(ts, session, name) {
        ...
        if (!name) {
            for (var i = 0;; ++i) {
                n = "[doc" + (i || "") +
                "]"; //name not passed for new doc, so auto generate it
                if (!ts.docs[n]) {
                    name = n;
                    break;
                }
    ...
    function getCallPos(editor, pos) {
        ...
        var iterator = new TokenIterator(editor.session, pos.row, pos
            .column);
        ...
        if (end.row == pos.row && end.column + token.value.length > pos
            .column) {
            token = {
                type: token.type,
                value: token.value.substring(0, pos.column - end
                    .column)
            };
        }
        var type;
        ...
        function time() {
            if (++a % 10) {
                ...
                if (t > timeOut) throw new Error(
                    "Timed Out getting call pos");
            }
            return true;
        }
        while (token && iterator.getCurrentTokenRow() > maxLine &&
            time()) {
            type = "." + token.type + ".";
    ...
    function docValue(ts, doc) {
        ...
        if (ts.options.fileFilter) val = ts.options.fileFilter(val, doc
            .name, doc.doc);
        return val;
    }

    function somethingIsSelected(editor) {
        return editor.getSession().getTextRange(editor
            .getSelectionRange()) !== '';
    }
    ...
    function moveTo(ts, editor, curDoc, doc, pos, doNotCloseTips) {
        if (curDoc != doc) {
            if (ts.options.switchToDoc) {
                ...
                ts.options.switchToDoc(('/' + doc.name).replace("//",
                        "/"), null, null, doc.doc,
                    function( /*session*/ ) {
    ...
    function setSelection(ts, editor, pos) {
        ...
        if (start) {
            editor.gotoLine(start.row, start.column ||
            0); //this will make sure that the line is expanded
        ...
        if (ts.lastJumpPosition.isEmpty()) {
            ...
            ts.lastJumpPosition.extend(wordRange.start.row, wordRange
                .start.column);
            ts.lastJumpPosition.extend(wordRange.end.row, wordRange.end
                .column);
        }
    }
    ...
    function isOnFunctionCall(editor) {
        ...
        return false; //function definition
        if (tok.type.indexOf('storage.type') !== -1)
            return false; // could be 'function', which is start of an anon fn
        var nextTok = editor.session.getTokenAt(editor
            .getSelectionRange().end.row, (tok.start + tok.value
                .length + 1));
    ...
    function createCommands(proto, prefix) {
        ...
        if (proto.refresh) commands.refresh = {
            ...
            exec: function(editor) {
                ...
                if (editor[name].refreshDocLastCalled != null) {
                    if (new Date().getTime() - editor.tsClient
                        .refreshDocLastCalled <
                        1000) { //less than 1 second
                        full = true;
                    }
                }
                editor[name].refreshDocLastCalled = new Date()
                    .getTime();
        ...
        require("grace/core/config").Config.registerValues(noConf,
            "keyBindings");
