...
modes: list of modes where it is enabled"
    ...
    **/
    LspTransport.prototype.postMessage = function(data, cb) {
        ...
        if (this.transformCalls) throw new Error(
            'Unsupported operation');
        else ajax(
            this.ctx.address + "/" + data.type, {
                data: data,
    ...
    var LspClient;
    LspProvider.prototype.init = Depend.define(function(cb) {
            require(["./ext/autocompletion/lsp/lsp_client"],
                function(mod) {
        ...
        function(editor, cb) {
            ...
            self.transport.postMessage({
                ...
                self.instance = new LspClient(self
                    .capabilities, self.transport);
                return cb(self.instance);
            });
        });
    ...
    function update(ev) {
        if (ev.config == 'lspConfigs') {
            ...
            added.map(toConfig).forEach(function(e) {
                exports.createLspProvider(e);
            });
        }
    }
    exports.createLspProvider = function(config) {
        if (providers[config.name])
            completions.removeCompletionProvider(providers[config
                .name]);
        if (providers[config.name] && config.address == providers[
                config.name].address && config.method ==
            providers[config.name].method) {
            var provider = providers[config.name];
            provider.modes = config.modes;
        } else {
            providers[config.name] = new LspProvider(config.address,
                config.method, config.modes);
        }
        completions.addCompletionProvider(providers[config.name],
            providers[config.name].modes);
    };
    update({
        config: 'lspCconfigs',
        oldValue: [],
    ...
    require("grace/core/config").ConfigEvents.on("autocompletion",
        update);
});