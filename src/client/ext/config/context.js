define(function (require, exports, module) {
    ...
    function setupNode(e) {
        ...
        e._id || Utils.defProp(e, '_id', 'r' + _id++);
        Utils.defProp(e, '_true');
        //Stores nodes,counters for binary ops, regex for globs etc.
        Utils.defProp(e, '_cache');
        Utils.defProp(e, '_parent');
    ...
    Context.refresh = function (name, value, force) {
        ...
        if (c > 0 || force)
            Configs.withErrorHandler(Notify.inform, Configs.commit);
    };
    ...
    Context.GLOB = function (glob, path, cache, name) {
        ...
        return (
            ...
            cache.glob.test(
                FileUtils.relative(FileUtils.getProject().rootDir, path)
            )
        );
    };
    ...
    function _checkGlobs() {
        ...
        nodes.forEach(function (e) {
            if (
                e.rule.op === '=~' &&
                updateNode(e, Context.getContext(e.rule.ctx))
            )
                c++;
        });
