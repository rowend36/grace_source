define(function(require, exports, module) {
    /*globals $*/
    require("grace/ext/ace_loader!ace/ext/options");
    ...
    SettingsPanel.renderOption = function(key, option) {
        if (option.path && !this.editor.$options[option.path] && !
            option.onchange) return;
        this.options[option.path] = option;
        var safeKey = "-" + option.path;
        key = sentenceCase(key);
        var control = this.renderOptionControl(safeKey, option);
        if ((option.type !== "checkbox" && option.type) || Array
            .isArray(option) || option.items) {
            return [
                "tr",
                {
    ...
    SettingsPanel.on("setOption", function() {
        for (var i in docs) {
            if (docs[i].constructor == require("./settings_doc")
                .SettingsDoc) {
                docs[i].setDirty();
            }
        }
    ...
    FileUtils.on("sidenav-open", function() {
