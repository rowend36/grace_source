define(function(require,exports,module) {
    var nav = require("grace/core/navigation").Navigation;
    ...
    var skip = function(e) {
        ...
        clearSkip();
        if (!(e.shiftKey || e.ctrlKey || e.altKey) && (key == keys.esc || /*direction keys 40-37*/ (key <
                41 && key >
                36))) { //(key == direction + 1 || key == direction - 1)) {
    ...
    function trackType(el) {
        var group = new Utils.eventGroup(el);
        ...
        function validate(ev) {
            if (!(ev.shiftKey || ev.ctrlKey || ev.altKey)) {
                ...
                if (key == 229 || (keys.PRINTABLE_KEYS[key] &&
                        (key !== 190 && (key < 48 || key > 57)))) {
                    ev.preventDefault();
                }
            }
        }
    ...
    Object.assign(nav, {
        moveTo: function(a, e, source) {
            ...
            if (FocusManager.isFocusable(a)) {
                ...
                if (!source || Env.isHardwareKeyboard || (source != 'mouse' && FocusManager
                        .keyboardVisible)) {
        ...
        onenter: function(el, e) {
            if (FocusManager.isFocusable(el)) {} else if (el.tagName == 'SELECT') {
        ...
        addRoot: function(el, close, trap) {
            if (this.stack) {
                ...
                if (firstElement)
                    this.setFocused(firstElement, "root");
    ...
    var behaviours = nav.defaults = {
        "tabs": {
            ...
            getNext: function(current, root, dir, $default) {
                var isHorizontal = this.root.clientWidth > this.root.clientHeight;
                if (dir == 'horizontal' && isHorizontal ||
        ...
        "fileview": {
            ...
            detect: function(node) {
                return (" " + node.parentElement.className + " ").indexOf(" fileview ") > -1 && node
                    .parentElement;
            }
        }
    };
