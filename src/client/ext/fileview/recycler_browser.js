define(function (require, exports, module) {
  ...
  RecyclerStub.prototype.updateVisibleItems = function (force) {
    if (force) {
      for (var i = Math.min(this.pageEnd, this.pageSize) - 1; i >= 0; i--) {
  ...
  RecyclerStub.prototype.renderView = function (index, view) {
    view.attrs = {};
    view.attr('filename', this.names[index]);
    ...
    if (isDirectory(filename)) {
      ...
      if (cbs) {
        if (cbs[filename] && !cbs[filename].renderer.hidden) {
          ...
          Icons.renderEl(view.find('.type-icon'), 'folder_close', filename);
  ...
  FileItemHolder.prototype.attr = function (text, value) {
    if (arguments.length === 1) {
      ...
      } else if (this.boundView) this.boundView.attr(text, value);
      if (text == 'class') {
  ...
  RecyclerNavBehaviour.prototype = {
    ...
    getNext: function (/*current, root, dir, $default*/) {
      ...
      var index = parent.names.indexOf(parent.filename(this.stub.rootDir));
...
*/
  ...
  RecyclerNestedBrowser.prototype.expandFolder = function (name, callback) {
    ...
    var callback2 = function () {
      ...
      if (callback) callback(this.nestedViews[name]);
    }.bind(this);
    ...
    if (!this.nestedViews[name]) {
      ...
      this.nestedViews[name] = new RecyclerNestedBrowser(
        child,
      ...
      this.nestedViews[name].renderer.show();
  ...
  RecyclerNestedBrowser.prototype.foldFolder = function (name) {
    this.mixinNestedBrowser.foldFolder.bind(this).apply(this, arguments);
