define(function (require, exports, module) {
    ...
    var appConfig = Config.registerAll({}, "files");
    var getObj = Config.getObj;
    var Notify = require("grace/ui/notify").Notify;
    var Form = require("grace/ui/form").Form;
    ...
    FileUtils.on("register-fileserver", function () {
        needsRecreate = true;
    });
    var _fileBrowsers = {};

    FileUtils.on("replace-fileserver", function (ev) {
        for (var i in _fileBrowsers) {
            if (_fileBrowsers[i].fileServer == ev.former) {
                _fileBrowsers[i].fileServer = ev.current;
                if (_fileBrowsers[i].tree) {
                    _fileBrowsers[i].tree.fileServer = ev.current;
                    for (var c in _fileBrowsers[i].tree.childStubs) {
                        _fileBrowsers[i].tree.childStubs[c].fileServer = ev.current;
                    }
                }
            }
        }
    ...
    var Fileviews = (exports.Fileviews = {
        ...
        createBrowser: function (id, fileServer, rootDir, icon) {
            Tabs.insertTab(0,
                id,
                icon || (fileServer || FileUtils.defaultServer).icon || "sd_storage",
            );
        ...
        pickFile: function (info, end, allowNew) {
            ...
            var grp = new Utils.eventGroup(FileUtils);
            var dismiss = info ? Notify.direct(info) : Utils.noop;
            grp.on("clear-temp", dismiss);
        ...
        saveAs: function (doc) {
            ...
            Fileviews.pickFile("Select a file to overwrite or create a new file", onSave, true);
        },
        inSaveMode: function () {
            return saveMode;
        },
