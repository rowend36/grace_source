define(function(require, exports, module) {
    ...
    var ProjectView = require("grace/ext/fileview/recycler_browser").RProjectView || require(
        "grace/ext/fileview/file_browser").ProjectView;
    var bind = require("grace/ui/ui_utils").createSearch;
    var FindFileServer = require("grace/ext/fileserver/find_fs").FindFileServer;
    ...
    var AutoCloseable = require("grace/ui/autocloseable").AutoCloseable;
    ...
    var SearchTab = function(el) {
        ...
        appConfig.lastSearchOpts = parseInt(appConfig
            .lastSearchOpts);
        if (appConfig.lastSearchOpts & 4) regExpOption.addClass(
            'checked');
        if (appConfig.lastSearchOpts & 2) caseSensitiveOption
            .addClass('checked');
        if (appConfig.lastSearchOpts & 1) wholeWordOption.addClass(
            'checked');
        ...
        var searchInFolder = this.searchInFolder = function(folder,
            server) {
            project = {
                rootDir: folder,
                fileServer: server
            ...
            if (self.previewBrowser) {
                self.previewBrowser.fileServer = project
                    .fileServer;
        ...
        function filterFiles(name, path) {
            ...
            if (appConfig.matchBaseName) {
                return (isDirectory || (!includeFilter ||
                    includeFilter.test(name) ||
                    includeFilter
                    .test(rel))) && !(
                    excludeFilter && (excludeFilter.test(
                        name) || excludeFilter.test(rel)));
            }
            return (isDirectory ? quickFilter.test(rel) : (!
                includeFilter || includeFilter.test(
                    rel))) && !(
        ...
        this.init = function() {
            ...
            if (project.fileServer) {
                searchInFolder(project.rootDir, project
                    .fileServer);
            }
            includeFilter = appConfig.searchTabFilter &&
                globToRegex(appConfig.searchTabFilter);
            quickFilter = genQuickFilter(appConfig
                .searchTabFilter);
            excludeFilter = globToRegex(appConfig
                .searchTabExclude);
        };
        ...
        var openModal = function() {
            ...
            if (!modalEl) {
                ...
                self.previewBrowser = new ProjectView(modalEl
                    .find(".fileview"), project.rootDir,
                    project
                    .fileServer);
                self.previewBrowser.setShowFileInfo(false);
                self.previewBrowser.setRootDir(project.rootDir);
                self.previewBrowser.folderDropdown =
                    "search-dropdown";
                self.previewBrowser.childFolderDropdown =
                    "search-dropdown";
                self.previewBrowser.fileDropdown =
                    "search-file-dropdown";
                self.previewBrowser.foldersToIgnore = [];
                self.previewBrowser.menuItems = {
                    "search-dropdown": Object.create(self
                        .previewBrowser.menuItems[self
                            .previewBrowser
                ...
                searchInTabs = modalEl.find("#includeOpenDocs")[
                    0];
                $(searchInTabs).on("change", function(e) {
                    ...
                    modalEl.find('input[type=text]')
                        .attr('disabled', appConfig
                            .searchOpenDocs);
                    searchOpenDocs(val);
                });
                modalEl.modal({
                    ...
                    onCloseStart: function() {
                        ...
                        if (self.SearchList) self
                            .SearchList.reset(true);
                        self.previewBrowser
                            .inFilter && self
                            .previewBrowser
                            .stopFind();
            ...
            var dotstar = require("grace/core/config").Config.allConfigs.files
                .dotStar;
            if (dotstar) {
                modalEl.find("#dot-star-status").html(
                    'enabled');
            } else
                modalEl.find("#dot-star-status").html(
                    'disabled');
            ...
            modalEl.find('input[type=text]').attr('disabled',
                appConfig.searchOpenDocs);
        ...
        var updatePreview = function(ev) {
            ...
            if (this == filterInput[0]) {
                ...
                configure("searchTabExclude", $(this).val(),
                    "search");
            }
            ...
            updateTimeout = (ev ? setTimeout : setImmediate)(
                function() {
                    self.previewBrowser.findFile(
                        filterFiles);
                    updateTimeout = null;
                }, 2000);
        };
        ...
        var beginSearch = function(filename, useServer) {
            var doc = Docs.forPath(filename, (useServer ===
                false) ? undefined : (project && project
                .fileServer));
            if (doc) {
            ...
            } else(useWorker ? workerSearch : asyncSearch)(
                filename, project.fileServer,
                currentTimeout,
                searchId);
        };
        ...
        var find = function() {
            ...
            var opts = (regExpOption.hasClass("checked") ? 4 :
                0) + (caseSensitiveOption
                .hasClass(
                    "checked") ? 2 : 0) + (wholeWordOption
            ...
            searchInfo.text("");
            try {
                self.search.$assembleRegExp(self.search
                    .$options);
            } catch (e) {
                showSearchStarted();
                return showSearching(" ",
                    "<span class='error-text'>" + e
                    .message + "</span>");
            }
            useWorker = appConfig.useSearchWorker;
            configure("lastSearch", self.searchInput.val(),
                "search");
            configure("lastSearchOpts", opts, "search");
            searchTimeout = Utils.parseTime(appConfig
                .searchTimeout);
            showSearchStarted();
            if (!project || appConfig.searchOpenDocs) {
                ...
                if (rootDir != self.rootDir || !this
                    .SearchList) {
                    var server = new FindFileServer(project
                        .fileServer);
                    server.setFilter(filterFiles);
                    self.SearchList = new SearchList(rootDir,
                        server);
                    self.SearchList.onDir = onDir;
                    self.rootDir = rootDir;
                }
                self.SearchList.reset();
                currentTimeout = new Date().getTime() +
                    searchTimeout;
                for (var i = 0; i < 10; i++) {
                    runningSearches++;
        ...
        function renderResults(doc, ranges) {
            ...
            if (ranges && ranges.length > 0) {
                found += ranges.length;
                searchInfo.text('Found ' + found + ' results in ' +
        ...
        find = require("grace/core/depend").define(
            function(cb) {
                require([
                        "./search_list",
                        "./search_results",
                    ...
                    function(_searchList, _searchResults, _searchReplace) {
                        ...
                        searchResults = new SearchResults(el.children(
                            "#searchResults"), useRecycler);
                        ...
                        searchResults.afterLineClicked = function() {
                            getEditor().session.highlight(self
                                .search.getOptions().re);
                        };
                        searchReplace = new SearchReplace({
                            ...
                            getReplacer: self.search.replace
                                .bind(self.search)
                        });
                        cb();
                    });
        ...
        var replace = function() {
            if (searchedDocs.length) {
                ...
                Notify.ask("Replace all instances of " +
                    lastReplaced + " with " + lastReplace +
                    "?",
                    function() {
                        searchReplace.setServer(FileUtils
                            .getFileServer(
                                lastSearchServer));
                        undoButton.show();
                        searchReplace.replace(searchedDocs,
                            lastReplace);
                    });
            }
        };
        var undoReplace = function() {
            ...
            if (a && a.length) {
                Notify.ask("Undo replacement of " +
                    lastReplaced + " with " + lastReplace +
                    "?",
                    function() {
                        searchReplace.undo();
        ...
        function possibleNewSearch() {
            if (runningSearches && runningSearches < (useWorker ?
                    5 : 2) && found < targetFinds &&
                new Date().getTime() <
                currentTimeout) {
                runningSearches++;
                self.SearchList.getNext(beginSearch, empty);
        ...
        function clearDocs(e) {
            for (var i in docs) {
                if (docs[i].searchWillClear) {
                    if (docs[i].bound) docs[i].searchWillClear =
                        false;
                    else closeDoc(i);
                }
            }
        ...
        function handleResult(e) {
            ...
            switch (e.data.message) {
                case 'results':
                    ...
                    if (ranges.length) {
                        showSearching(doc.getPath(), "Found " +
                            ranges.length + " results");
                        searchedDocs.push(doc.getSavePath());
                    }
                    possibleNewSearch();
                    renderResults(doc, ranges);
                    if (doc.searchWillClear && !doc.bound) closeDoc(
                        doc.id);
                    break;
                case 'getFile':
                    searchWorker.postMessage({
                        re: self.search.$options.re || self
                            .search.$assembleRegExp(self
                                .search
                                .$options),
                        id: doc.id,
                        text: doc.getValue(),
        ...
        function workerSearch(path, fileServer, timeout, id) {
            searchWorker = searchWorker || createSearchWorker(
                handleResult, clearDocs);
            terminate();
            var callback = function(doc, ck, clear) {
                if (!doc) {
                    if (ck == "binary" &&
                        ...
                        lastWarnedBinaryFile = id;
                    } else Notify.error("Error Reading " +
                        path);
                    possibleNewSearch();
                    renderResults();
                    return;
                ...
                if (!clear) {
                    if (appConfig
                        .syntaxHighlightSearchResults) {
                        mode = ace.require("ace/ext/modelist").getModeForPath(
                            path).mode;
                    }
        ...
        function asyncSearch(path, fileServer, timeout, id) {
            ...
            var search = function() {
                var ranges = self.search.findAll(doc.session);
                if (ranges.length) {
                    showSearching(path, "Found " + ranges
                        .length + " results");
                    searchedDocs.push(doc.getSavePath());
                }
                possibleNewSearch();
            ...
            fileServer.readFile(path, FileUtils.encodingFor(path,
                fileServer), function(err, res) {
                if (id != searchId) return;
                ...
                try {
                    ...
                    console.error(
                        e); //new Error('Error during async search'))
                }
                closeDoc(doc.id);
            });
        }
        ...
        el.find("#toggleReplace").click(
            function() {
                ...
                $(this).toggleClass('btn-toggle__activated', el
                    .hasClass('show_replace'));
            });
        ...
        FileUtils.on(
            'change-project',
            function(e) {
                searchInFolder(e.project.rootDir, e.project
                    .fileServer);
            });
        var searchInFolderOption = {
            ...
            onclick: function(ev) {
                searchInFolder(ev.filepath, ev.browser
                    .fileServer);
        ...
        FileUtils.registerOption("files", ["folder", "create"],
            "search-in-folder", searchInFolderOption);
        $("#search_tab").show();
        //endregion
    };
...
<ul class="fileview">\
    ...
    var createSearchWorker = function(cb, e) {
        if (!WORKER_BLOB_URL) WORKER_BLOB_URL = URL.createObjectURL(
            new Blob(["(" + inlineWorker.toString()
                .replace(
        ...
        function inlineWorker() {
            ...
            function cache(path, res) {
                ...
                if (cachedSize > MAX_CACHE_SIZE) {
                    ...
                    keys.sort(function(e, l) {
                        return cached[l].length - cached[e]
                            .length;
                    });
            ...
            self.onmessage = function(e) {
                if (e.data.hasOwnProperty('text')) {
                    ...
                    if (text === null || text.length != e.data
                        .len) {
                        self.postMessage({
                            id: e.data.id,
                            message: 'getFile'
    ...
    var SearchPanel = new exports.SearchTab(
        $("#search_container"),
        $("#searchModal")
    );
