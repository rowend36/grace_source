define(function (require, exports, module) {
    ...
    function createHeader(noRecycle) {
        ...
        div.innerHTML =
            "<h6 class='clipper searchResultFile'></h6>" +
    ...
    function withRecycler(el, renderer, ctx) {
        ...
        getHolder = function (doc, ranges) {
            ...
            if (
                doc.session.getModeName() !== 'text' &&
                mode &&
                ranges[ranges.length - 1].end.row < MAX_KEEP_LINES_RANGE_LENGTH
            ) {
                ...
                for (var i in ranges) {
                    ...
                    if (start == endLine) {
                        ...
                        value.push.apply(
                            value,
                            doc.session.getLines(start, endLine)
                        );
                        endLine++;
                    }
                }
            ...
            var session = new EditSession(lines);
            session.$stoppedWorkers = true;
            ...
            var holder = Object.assign(
                {
                    ...
                    start: 0,
                    hidden:
                        ranges.length > ctx.searchConfig.resultsAutoShowLines,
                    views: [],
                },
                holder_props
            );
    ...
    ResultsView.prototype.onLineClicked = function (el) {
        switchToDoc(
            el.searchData.path,
            el.searchData.range.start,
            el.searchData.range.end,
            true,
            this.afterLineClicked,
            this.server
        );
    };
    ...
    if (RecyclerViewHolder) {
        ...
        HeaderViewHolder.prototype = Object.create(
            RecyclerViewHolder.prototype
        );
        HeaderViewHolder.prototype.bindView = function () {
            ...
            this.view
                .find('.foldResult')
                .toggleClass('btn-toggle__activated', this.holder.hidden);
        };
        ...
        var ResultViewHolder = function (holder, renderer) {
            ...
            for (
                var i = start;
                this.height < holder.maxHeight && i < holder.ranges.length;
                i++
            ) {
                this.height +=
                    this.range[i].end.row - this.range[i].start.row + 1;
            }
        ...
        ResultViewHolder.prototype = Object.create(
            RecyclerViewHolder.prototype
        );
        ...
        ResultViewHolder.prototype.bindView = function () {
            //quite unfortunate,
            $(this.view[0]).empty();
            this.view[0].className =
                this.holder.config.themeClass + ' border-secondary';
            ...
            this.holder.renderer.render(
                ranges,
                this.holder.session,
                null,
                this.view[0],
                this.holder.lastLine
            );
