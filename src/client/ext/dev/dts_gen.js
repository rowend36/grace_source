  ...
  */
  ...
  function addGlobalObjects() {
    for (var i in window) {
      if (
        window[i] &&
        (typeof window[i] === 'function' || typeof window[i] === 'object')
      )
...
*/
    ...
    if (typeof e === 'function') {
      if (
        ...
        cursor(
          ...
          e.name,
          m ? ' extends ' + m.name : '',
          n.names.length ? '/* mixins ' + n.names.join(', ') + ' */' : '',
          ' {',
        );
        var hasStatic = false;
        for (var i in e) {
  ...
  function printFunc(e, depth, cursor) {
      ...
      function (node) {
        ...
        if (inBody) {
          if (
            ...
            if (node.depth < inNestedFunction - 1) inNestedFunction = 0;
          ...
          if (node.type === 'RETURNED') {
            if (inNestedFunction && node.depth >= inNestedFunction) return;
            ...
            */
            ...
            while ((prev = prevSibling(prev))) {
              ...
              if (path.length === 0) {
                var varTypes = context[prev.text] || (context[prev.text] = []);
                varTypes.push(type);
                break;
              }
            }
    ...
    args.forEach(function (e, i) {
      ...
      if (!context[e] && returnTypes.indexOf('@' + e) > -1) {
        var Letter = String.fromCharCode('T'.charCodeAt(0) + (i - 1) / 3);
    ...
    if (returnTypes.length) {
      ...
      if (!returnType || returnType === 'null' || returnType === 'undefined')
        returnType = 'any';
    }
  ...
  function safe(str) {
    return /^[_a-zA-Z\$][a-zA-Z\$_0-9]*$/.test(str) ? str : JSON.stringify(str);
  }
  function getSuper(e) {
    var m = Object.getPrototypeOf(e.prototype);
    if (
      e.super ||
      (m && m.constructor !== Object && isConstructor(m.constructor || {}))
    ) {
    ...
    if (props.length) {
      ...
      do {
        ...
        props = props.filter(function (prop) {
          return /[a-zA-Z]/.test(prop[0]) && text.indexOf('this.' + prop) < 0;
        });
      } while ((m = getSuper(m)));
      if (props.length) {
