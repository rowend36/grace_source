define(function () {
  ...
  require(['grace/core/app_events', 'grace/core/utils'], function (e, u) {
    function log(name) {
      ...
      console.debug(
        name + ' (' + loaded + ' modules): ' + ' size =' + u.Utils.toSize(size),
  ...
  Object.defineProperty(window, 'unused', {
    get: function () {
      ...
      return {
        ...
        execEfficiency:
          Math.round((used.length / (waiting.length + used.length)) * 100) +
          '%',
      };
    },
  });
  ...
  function Inspector(func) {
    ...
    this.exec = {
      [func.name]: function (...args) {
        var isConstructor = Object.getPrototypeOf(this) === self.exec.prototype;
        var ret = isConstructor ? new func(...args) : func.apply(this, args);
  ...
  Inspector.prototype.update = function (args, ret) {
    ...
    if (l > 0 && this.cb) this.cb(this.args.map(toStr), [], toStr(this.ret));
  };
  ...
  function toStr(e) {
    ...
    return e.join('|').replace('|?', '|undefined').replace('?', 'any') || 'any';
  }
});