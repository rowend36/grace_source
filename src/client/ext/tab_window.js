define(function(require, exports, module) {
    ...
    var setTab = require("../setup/setup_tab_host").setTab;
    var DocsTab = require("../setup/setup_tab_host").DocsTab;
    var getEditor = require("../setup/setup_editors").getMainEditor;
    ...
    */
    ...
    exports.getTabWindow = function(id, name, info, onClose, insert,
        host) {
        if (!doneInit)
            ViewPager.init();
        ...
        pager.add(id, host);
        if (insert !== undefined) {
            DocsTab.insertTab(DocsTab.indexOf(insert) + 1, id, name,
                null,
                info);
        }
    ...
    ViewPager.init = function() {
        ...
        app.on('closeTab', function(e) {
            ...
            if (host) {
                if (host.autoClose || (host.onClose && host
                        .onClose() !==
                        false))
                    exports.closeTabWindow(id);
                e.preventDefault();
        ...
        app.on('createEditor', function(e) {
            if (e.isMain && !e.editor.viewPager)
                e.editor.viewPager = new ViewPager(e
                    .editor);
        });
    ...
    exports.getHostEditor = function(editor) {
        return doneInit ? editor.viewPager && editor.viewPager
            .mainEditor : editor;
    };
}); /*_EndDefine*/