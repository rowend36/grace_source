define(function(require, exports, module) {
    ...
    TabRenderer.prototype.createItem = function(id, name) {
        return (
            '<li draggable=true class="' + this.TAB_ITEM_CLS +
            '" ' +
            this.ID_ATTR + '=' +
            id +
            "><a href=#" +
    ...
    TabRenderer.prototype.scrollIntoView = function(el) {
        ...
        menu.scrollLeft = Math.max(
            0,
            -menu.clientWidth / 2 + child.clientWidth / 2 +
            child.offsetLeft
        );
    };
    ...
    TabRenderer.prototype.setFadeCloseIcon = function() {
        ...
        var $el = this.$el;
        $el
            .on("mouseover", "." + this.TAB_ITEM_CLS, function() {
                $($el)
                    .find(m)
                    .hide();
                $(this)
    ...
    TabRenderer.prototype.getTabEl = function(id) {
        return this.$el.find("." + this.TAB_ITEM_CLS).filter("[" +
            this.ID_ATTR + "=" +
            id + "]");
    };
    TabRenderer.prototype.getTabId = function(el) {
        return el.closest("." + this.TAB_ITEM_CLS).attr(this
            .ID_ATTR);
    };
    ...
    TabRenderer.prototype.getOnClickListener = function(host, scroll) {
        ...
        return function(e) {
            ...
            var target = $(e.target);
            var isClose = target.closest("." + self
                .CLOSE_BTN_CLS);
            if (isClose.length) {
                e.preventDefault();
                host.onClose(id);
