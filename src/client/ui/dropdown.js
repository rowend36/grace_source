define(function(require, exports, module) {
  ...
  var AutoCloseable = require("./autocloseable").AutoCloseable;
  ...
  function Dropdown(keepFocus, align, vAlign) {
    ...
    self.onOverlayClick = function(e) {
      var parent = closest(document.elementFromPoint(e.clientX, e
        .clientY));
      while (current != root && getElement() != parent) {
        hideOverflow(e);
      }
    ...
    function toggle(e) {
      if (root) {
        if (nav.length < 1) showOverflow("root", e.currentTarget);
        else {
          closeable.close();
        }
      }
    ...
    self.setData = function(w, changed) {
      ...
      if (inRecreate) {
        for (var j = 0; current[store[j]] && j < store.length; j++) {
          showOverflow(store[j], j < 1 ? anchor : $(getElement()).find(
            "#" + store[j])[0]);
        }
        inRecreate = false;
        if (nav.length === 0) {
  ...
  Dropdown.defaultLabel = function(label) {
    return {
      isHeader: true,
      className: "mt-10",
      caption: "<label class='sub-header'>" + Utils.htmlEncode(label) +
        "</label>",
    };
  };
    ...
    if (sorted[0][0] && sorted[0][0].isHeader) {
      sorted.sort(function(a, b) {
        ...
        return items[a].isHeader ?
          ...
          l < t ?
          1 :
          (items[a].caption || items[a]).localeCompare(items[b]
            .caption || items[b]);
      });
    }
   ...
   * @param {HTMLElement} inside - the bounding container
   * @param {(right|left|full|center|responsive-right)} [align=Dropdown#defaultAlignment] - horizontal alignment
   * @param {boolean} allowToRight - allow desktop stacking behaviour on wide screens
   * @param {(above|below|above_or_below|full|max)} [align='above_or_below'] - horizontal alignment
   **/
  Dropdown.positionElement = function(element, anchor, inside, align,
    allowToRight, vertAlign) {
    ...
    var preferBelow = maxH - (anchorTop + anchorH) > (anchorBottom -
      anchorH) * 0.8;
    ...
    if (vertAlign == "below" || (preferBelow && vertAlign === "max")) {
      ...
      } else if (vertAlign !== 'max' || maxH - anchorBottom > 250) {
    ...
    if (align == "right" || (align == "responsive-right" && window
        .innerWidth > 400)) {
      element.style.right = maxW - Math.max(w + 10, anchorRect.right -
        10) + "px";
      element.style.left = "auto";
    } else if (align == "left") {
      element.style.left = Math.min(anchorRect.left + 10, maxW - w - 10) +
        "px";
  ...
  Dropdown.openSelect = function(ev, select) {
    if (ev) {
      ...
      ev.preventDefault();
      if (!require("../core/focus_manager").FocusManager
        .keyboardVisible) {
        select.focus();
      }
    }
    ...
    for (var i = 0; i < opts.length; i++) {
      if (opts[i].tagName == "OPTGROUP") {
        items.push({
          isHeader: true,
          className: i ? "mt-10" : "",
          caption: "<label class='sub-header'>" + opts[i]
            .getAttribute("label") + "</label>",
        });
      } else {
        items.push({
          value: opts[i].value,
