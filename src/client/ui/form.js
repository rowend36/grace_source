define(function(require, exports, module) {
    ...
    function createForm(form, el) {
        ...
        require("../ui/ui_utils").styleCheckbox($(el));
        return el;
    }
    ...
    function createElement(t, el) {
        ...
        } else input = document.createElement(t.type == 'select' ?
            'select' : 'input');
        if (t.id) throw new Error(
            'Extraneous property id, do you mean name?');
        if (t.name) {
        ...
        switch (t.type) {
            ...
            case 'button':
                ...
                btn.setAttribute('value', t.caption || sentenceCase(
                    name));
                btn.setAttribute('type', t.type);
                if (t.onclick)
                    $(btn).on('click', t.onclick);
                el.appendChild(btn);
                break;
            case 'accept':
            case 'checkbox':
                ...
                var span = document.createElement(t.type ===
                    'checkbox' ? 'span' : 'label');
    ...
    function parseForm(e) {
        ...
        for (var i = 0; i < children.length; i++) {
            var conf = children[i];
            params[conf.name] = conf.type == 'checkbox' ? conf.checked :
                conf.value;
        }
