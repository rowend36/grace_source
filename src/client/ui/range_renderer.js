define(function(require,exports,module) {
    ...
    (function() {
        ...
        this.Text = renderer;
        this.clipText = "...";
        this.Clip = function(start, tokens, clipText, end,
        nomodify) {
        ...
        this.render = function(ranges, session, config, parent,
            maxLine) {
            ...
            var clip = function(row, tokens) {
                ...
                return clipper(start, tokens, clipText, cw *
                    1.5, true);
            };
            var size = 0;
            var start = 0;
            ...
            var max_line = maxLine || ranges[ranges.length - 1]
                .end.row;
            ...
            for (var rangeId in ranges) {
                ...
                var isSingleLine = range.start.row == range.end
                    .row;
                var line = session.getLine(range.start.row);
                size =
                    (isSingleLine ? range.end.column : line
                        .length) -
                    range.start.column;
                var free_space = cw - size;
                if (isSingleLine) {
                    var m = free_space / 2;
                    var endSpace = Math.min(m, line.length -
                        range.end.column);
                ...
                for (var i = range.start.row + 1; i <= range.end
                    .row; i++) {
                    lineEl = document.createElement("div");
                    lineEl.style.lineHeight = lineEl.style
                        .height = config.lineHeight + "px";
                    lineEl.style.padding = config.padding +
                    "px";
                    gutterEl = document.createElement("span");
                    gutterEl.appendChild(document
                        .createTextNode(gutter));
                    gutterEl.setAttribute("id", "line-number");
                    lineEl.appendChild(gutterEl);
                    gutterEl.style.position = "static";
                    lineEl.appendChild(gutterEl);
        ...
        this.renderPlain = function(ranges, session, config, parent,
            maxLine) {
            config = config || this.config;
            session = session || this.session;
            if (!parent) {
            ...
            for (var rangeId in ranges) {
                ...
                var lines = session.getLines(range.start.row,
                    range.end.row);
                parent.append(
                ...
                if (lines.length == 1) {
                    ...
                    result = lines[0].substring(range.start
                        .column);
                    elRes.appendChild(document.createTextNode(
                        safe(result)));
                    $(elRes).append(document.createTextNode("&nbsp;"));
                    rw =
                        elRes.getBoundingClientRect().right -
                        elRes.getBoundingClientRect().left;
                    elRes.appendChild(document.createElement(
                        "br"));
                    ...
                    for (var i = 1; i < lines.length - 1; i++) {
                        ...
                        $(elRes).append(document.createTextNode("&nbsp;"));
                        elRes.appendChild(document
                            .createElement("br"));
                        $(elRes).append(document.createTextNode("&nbsp;&rdsh;"));
                    }
                    elRes.appendChild(
                        document.createTextNode(
                            lines[i].substring(0, range.end
                                .column)
                        )
                    );
                }

                var start = lines[0].substring(0, range.start
                    .column);
                var free_space = Math.floor((cw - rw) / (rw /
                    result.length));
                var s = 0,
                    m = free_space / 2;
                var end = lines[lines.length - 1].substring(
                    range.end.column);
                s = Math.min(
                    start.length,
                ...
                parent.children().last().children("#end").text(
                    safe(end));
                parent.addClass("ace_editor");
                parent.children().addClass("ace_line");
                parent.children().css("line-height", config
                    .lineHeight + "px");
                parent.children().css("height", config
                    .lineHeight + "px");
                parent.children().css("padding", config
                    .padding + "px");
            }
        };
        
        //original source code
        renderer.$renderLine = function(parent, row, foldLine,
            multiplexer) {
            if (!foldLine && foldLine != false)
                foldLine = this.session.getFoldLine(row);

            if (foldLine) var tokens = this.$getFoldLineTokens(
                row, foldLine);
            //the differernce --->
            else var tokens = multiplexer(row, this.session
                .getTokens(row));
            var lastLineEl = parent;
            if (tokens.length) {
                var splits = this.session.getRowSplitData(row);
                if (splits && splits.length) {
                    this.$renderWrappedLine(parent, tokens,
                        splits);
            ...
            if (this.showInvisibles && lastLineEl) {
                ...
                var invisibleEl = this.dom.createElement(
                "span");
                invisibleEl.className =
                    "ace_invisible ace_invisible_eol";
