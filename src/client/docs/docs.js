define(function(require, exports, module) {
  ...
  var Docs = require("./document").$Docs;
    ...
    switch (ev.config) {
      case "clearCache":
        ...
        configure(
          ...
          Math.min(appConfig.nextCleanTime, new Date().getTime() +
            1000 * 60 * 60 * 24),
          "documents"
        );
        ev.preventDefault();
  ...
  function updateSizes() {
    maxStoreSize = Math.max(500000, Utils.parseSize(appConfig
      .maxStoreSize));
    maxDocCacheSize = Math.min(
      maxStoreSize / 10,
      Math.max(5000, Utils.parseSize(appConfig.maxDocCacheSize))
    );
    maxDocDataSize = Math.min(
      maxStoreSize,
  ...
  var historySaver = {
    ...
    $update: function(doc, res, revision) {
      ...
      var status = true;
      try {
        status = updateContentFromUndos(doc.session, doc.savedUndos,
          revision) || undefined;
      } catch (e) {
        debug.log(e);
      ...
      if (status) {
        ...
        state.folds.forEach(function(fold) {
          doc.session.addFold(fold.placeholder, Range.fromPoints(
            fold.start, fold.end));
        });
      }
      return status;
    ...
    refresh: function(doc, res) {
      ...
      var ERROR = false;
      /*if (doc.getSize()) {
          debug.error(new Error("Error: Unrefreshed doc modified"));
  ...
  function shrinkData(obj, targetSize) {
    ...
    do {
      if (obj.history.undo && obj.history.undo.length > 1) {
        obj.history.undo = obj.history.undo.slice(obj.history.undo.length /
          2);
  ...
  Docs.tempSave = function(id, force, cleaned) {
    ...
    if (!doc) return debug.warn("Temp saving closed doc");
    try {
      data = JSON.stringify(obj);
  ...
  Docs.saveAs = function(id, newpath, fileServer, callback) {
    ...
    if (doc.isTemp()) {
      ...
      id = addDoc("", "", newpath);
      Docs.$jsonToSession(docs[id].session, Docs.$sessionToJson(doc
        .session));
    }
    ...
    if (doc.encoding) {
      ...
      } else docs[id].encoding = typeof alias == "string" ? alias : doc
        .encoding;
    }
  ...
  Docs.rename = function(path, newpath, fs) {
    ...
    var affected = Object.keys(docs).filter(function(e) {
      ...
      if (save && (save == path || save.startsWith(path + sep))) {
        if (!fs || docs[e].getFileServer().getDisk() == fs.getDisk())
          return true;
      }
    });
    ...
    function doRename() {
      ...
      affected.forEach(function(e) {
        ...
        if (docpath.startsWith(path)) {
          docpath = doc.setPath(newpath + docpath.substring(path
            .length));
        }
  ...
  Docs.setEncoding = function(id, encoding) {
    ...
    if (alias) {
      ...
      if (!doc.isTemp()) {
        Notify.ask("Reload file with new encoding " + encoding + "?",
          function() {
            doc.refresh(
              function() {
      ...
      Notify.error("Encoding " + encoding +
        " not supported by this storage device");
    }
  };
  ...
  Docs.saveDocs = function(id, callback, force) {
    if (id !== undefined) {
      ...
      if (docs[id].duplicateId) {
        ...
        if (mainDoc) mainDoc.duplicateId = docs[id].duplicateId;
        if (mainDoc && mainDoc.allowAutoSave) {
          mainDoc.allowAutoSave = undefined;
          if (!mainDoc.warned) {
            mainDoc.warned = true;
            Notify.warn(
              "Saving Duplicate Document.</br> Autosave turned off.",
              1000);
          }
        }
        ...
        for (var k in docs) {
          var _doc = docs[k];
          if (_doc.duplicateId && _doc.getSavePath() == savePath) {
            _doc.setDirty();
          }
        }
      }
      ...
      for (var i in docs)
        if ((docs[i].dirty || force) && docs[i].allowAutoSave && !docs[i]
          .duplicateId)
  ...
  var autoSave = (Docs.$autoSave = Utils.delay(
    Docs.saveDocs,
    Math.max(Utils.parseTime(appConfig.autosaveInterval), 5000)
  ));
  ...
  Docs.forPath = function(path, server) {
    for (var i in docs) {
      if (docs[i].getPath() == path) {
        if (server && server.getDisk() !== docs[i].getFileServer()
          .getDisk()) {
          continue;
        }
        return docs[i];
  ...
  if (appConfig.nextCleanTime < time) {
    if (appConfig.nextCleanTime) {
      ...
      keys.forEach(function(e) {
        if (!blobRegistry[e]) appStorage.removeItem(e);
      });
    }
    configure(
      "nextCleanTime",
  ...
  var blobStorage = config.appStorage;
  var determineQuota = function(key) {
    ...
    try {
      ...
      var size = a.length / 2 + (a.length > 1000 ? determineQuota(key +
        1) : 0);
  ...
  Docs.hasBlob = function(id, name) {
    for (var i in blobRegistry) {
      if (i == "size") continue;
      if (blobRegistry[i].id == id && (!name || blobRegistry[i].type ==
          name)) {
        return i;
      }
    }
  ...
  Docs.initialize = function(tabs, activeTab) {
    Tabs = tabs;
    app.on("appPaused", function() {
      Docs.tempSave();
    });
    Tabs.registerPopulator("m", Docs);
    Tabs.fromJSON();
    //loading asynchronously is a big issue
    //because the state of Tabs changes, to do,
    //add a isLoadingparameter so that refresh happens
    ...
    Docs.fromJSON(null, null, activeTab, function() {
      ...
      //Docs.persist();
      Tabs.recreate();
      app.triggerForever("documentsLoaded");
    });
    Tabs.recreate();
  };
  ...
  function routineCheck(prefix) {
    ...
    keys = keys.filter(function(e) {
      return e.startsWith(prefix) && /^\d+$/.test(e.substring(prefix
        .length));
    });
    return keys;
  }
    ...
    if (!json) {
      ...
      if (failures || !json || Math.random() > 0.8) {
        ...
        if (keys.length) {
          ...
          return app.triggerForever("documentsLoaded");
        }
      }
    }
    ...
    function loadOne(id, index, next) {
      ...
      if (Breaks.hasFailures(id)) {
        Notify.ask(
          ...
          Docs.$loadData.bind(null, id, json[id], state),
          function() {
            var key;
    ...
    if (state) {
      if (state.factory) {
        if (__factory[state.factory]) {
          ...
          return FileUtils.postChannel("docs-" + state.factory, id, path,
            state);
        }
  ...
  function addDoc(name, content /*:string|Doc*/ , path /*:string?*/ ,
    opts) {
  ...
  function closeDoc(docId, keepUndos) {
    ...
    if (doc.bound && keepUndos && !doc.isTemp() && doc.getSavePath()) {
      Docs.stashDoc(doc.getSavePath(), JSON.stringify(defaultSaver.save(
        doc)));
    }
    if (appStorage.getItem(docId)) {
      appStorage.removeItem(docId);
    } else {
      if (doc.safe) debug.error("Unsafe doc with safe flag");
    }
    if (doc.bound) {
  ...
  Docs.openDoc = function(path, server, cb) {
    FileUtils.getDoc(path, server, function(doc, error) {
      ...
      (appConfig.autoCloseDocs ? Docs.createPlaceHolder : addDoc)(
        null, doc);
      doc.setClean();
      cb && cb(null, doc);
    });
  ...
  Docs.getName = function(id) {
    ...
    if (doc) {
      if (doc.isTemp()) {
        return "unsaved(" + (Tabs.indexOf(id) < 0 ? Tabs.numTabs() : Tabs
          .indexOf(id)) + ")";
  ...
  Docs.getInfo = function(id) {
    ...
    if (!docs[id].getSavePath()) {
      return "<i>" + docs[id].id + "</i>";
    }
    return Utils.htmlEncode(docs[id].getSavePath());
    //+"<i class='right' style='text-transform:uppercase'>"+docs[id].getEncoding()+"</i>";
  };

  var updateStatus = (Docs.$updateStatus = function(id) {
    if (docs[id])
      app.trigger("docStatusChange", {
        doc: docs[id]
      });
  });
  ...
  var inspectMemory = function() {
    var t =
      appStorage.getKeys && typeof appStorage.getKeys == "function" ?
      (function() {
        ...
        keys.forEach(function(e) {
          ...
          if (obj[e] && obj[e].length > 100) obj[e] = obj[e]
            .substring(0, 100) + "...";
        });
        return obj;
      })() :
