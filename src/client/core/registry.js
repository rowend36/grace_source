  ...
  function Registry(action, namespace) {
    ...
    function getProviderByName(name) {
      ...
      for (j = 0; j < providers.length; j++) {
        if (providers[j].names && providers[j].names.indexOf(name) > -1) {
          return providers[j];
        }
      }
    }
    ...
    function getActiveProviders(editor) {
      ...
      for (var j = 0; j < providers.length; j++) {
        if (providers[j] !== main && providers[j].modes.indexOf(mode) > -1) {
    ...
    function unregisterProvider(provider) {
      if (typeof provider === 'string') provider = getProviderByName(provider);
