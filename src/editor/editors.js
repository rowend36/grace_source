_Define(function(global) {
    ...
    function ScrollbarMarker(editor) {
        ...
        this.viewport = {
            ...
            update: function(html, self, session, config) {
                self.elt(
                    this.clazz,
                    "height:" + (self.editor.renderer.$size.scrollerHeight * config.scaleY) +
                    "px;" +
                    "width:" + config.width + "px;" +
                    "top:" + (self.editor.renderer.scrollMargin.top + session.getScrollTop()) *
                    config.scaleY + "px;" +
                    "left:" + 0 + "px;"
                );
            }
    ...
    ScrollbarMarker.prototype.scroll = function() {
        ...
        if (viewport) {
            viewport.style.top = (this.editor.renderer.scrollMargin.top + this.session.getScrollTop()) *
    ...
    ScrollbarMarker.prototype.updateCursors = function() {
        ...
        all.forEach(function(e, i) {
            ...
            this.cursors[i].style.cssText = "top:" + (this.$getTop(f.row, this.config) - height /
                2) + "px;left:" + left + "px;" + "height: " + height + "px;";
    ...
    ScrollbarMarker.prototype.render = function(changes) {
        ...
        var scaleX = (VSCROLL_WIDTH - 5) / Math.min(configOld.width, (r.$size.scrollerWidth * 2 + r
            .scrollLeft));
        var config = {
            scaleY: scaleY,
    ...
    Utils.inherits(ScrollbarMarker, Marker);
