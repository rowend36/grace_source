_Define(function(global) {
    ...
    global.registerValues({
            ...
            "wrap": {
                values: [
                    ["free", "use windowWidth as limit"], "printMargin", "off", "[number]"
                ]
            },
            "cursorStyle": {
    ...
    EditorSettings.prototype.setOption = function(key, val, noSave) {
        ...
        if (isSessionValue) {
            ...
            var isForAllEditors = (editors.length == 1 || !(key == "mode" || (appConfig
                .perEditorCompletion &&
                completionSettings.indexOf(key) > -1) || (appConfig.perEditorTheming && key ===
                "theme")));
            ...
            if (this.editor !== editors[0]) this.editor.setOption(key, val);
        }
    };
    ...
    var WRAP_MODE = new XOneOf([new XEnum(['off', 'free', 'printMargin', 'default']), IsNumber]);
    ...
    for (var i in optionsValidator) {
        ...
        if (s !== null && s !== undefined && s != "undefined" && s !== "null") {
            if (!optionsValidator[i].invalid(s)) {
                ...
                if (ace.Editor.prototype.$options[i] && ace.Editor.prototype.$options[i].forwardTo ===
                    "session") {
                    sessionDefaults[i] = s;
                } else {
                    editorDefaults[i] = s;
