_Define(function(global) {
    ...
    global.Caches = {
        ...
        get usage() {
            var sum = 0;
            var caches = global.Caches.caches;
    ...
    var poll = Utils.delay(function() {
        current++;
        if (global.Caches.usage) {
            poll();
        }
    }, 60000);

    global.createCacheFs = function(fs, cacheSize, noSave) {
        if (fs.cachedFs)
            return fs.cachedFs;
        else if (fs.isCached) return fs;
        var id = global.Caches.index++;
        ...
        var copyStat = global.FileUtils.createStats;
