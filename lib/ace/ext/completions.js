...
/*interface provider{
    ...
    destroy?: <unused>
    options: object
    embeddable: true,//can be used in embedded modes eg a javascript completer in html file
    isSupport: false,//to implement
    hasArgHints: true,//implements updateArgHints(editor)
    hasKeyWords: true,//completes keywords
    hasText: false,//completes random text
    hasStrings: false,//completes strings
    priority: 0,
    hasAnnotations: false,//implements updateAnnotations(editor)
    hasReferences: false //easy to implement using gotoLine
    hasDefinition: false //easy to implement using gotoLine
    name: string,
    port: //to implement either in app or as ace extension
}
*/
...
var onAfterExecDot = function(e, commandManager) {
    if (e.command.name === "insertstring" && e.args === ".") {
        ...
        if (tok && tok.type !== 'string' && tok.type.toString().indexOf('comment') === -1) {
...
var onCursorChange = function(e,editor) {
    ...
    debounceArgHints = setTimeout($onCursorChange.bind(editor),100);
};
...
var debounceAnnotations;
var onValueChange = function(e,editor) {
    clearTimeout(debounceAnnotations);
    debounceAnnotations = setTimeout($onValueChange.bind(editor), 100);
};
...
function updateAnnotations(editor,primary){
    if(primary && primary.hasAnnotations){
        if(!editor.$onSessionChange){
            editor.$onSessionChange = function(e){
                var session = e.session;
                if(session && session.$useWorker){
                    session.setUseWorker(false);
                }
                else return;
                session.$restartWorker = function(e){
                    if(e.editor && !e.editor.$onSessionChange){
            ...
            editor.$onSessionChange(editor);
            editor.on("changeSession",editor.$onSessionChange);
            editor.on("change",onValueChange);
        }
    }
    else if(editor.$onSessionChange){
        editor.off("change",onValueChange);
        editor.off("changeSession",editor.$onSessionChange);
        editor.$onSessionChange = null;
