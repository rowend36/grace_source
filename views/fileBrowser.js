(function(global) {
    ...
    FileBrowser.prototype.itemHeight = 32;
    ...
    FileBrowser.prototype.updateTopElements = function() {
        ...
        if (!this.backButton) {
            this.backButton = this.root.append(
                "<li filename=\"..\" class='tabbable file-item back-button ' tabIndex=0><span><i class = 'green-text material-icons'>reply</i></span><span class='filename'>" + ".." +
                "</span>" +
                "</li>").children().last();
    ...
    FileBrowser.prototype.updateBottomElements = function() {
        ...
        if (this.hier.length === 0) {
            this.bottomElement = this.bottomElement || this.root.append("<li class='fileview-footer'></li>").children().last();
            ...
            var mid = 'Page ' + (Math.floor(this.pageStart / this.pageSize) + 1) + ' of ' + (Math.floor(this.hier.length / this.pageSize) + 1);
    ...
    FileBrowser.prototype.renderView = function(i, view) {
        ...
        var color;
        if (filename == this.fileNameToSelect || (this.newFiles && this.newFiles.indexOf(filename) > -1))
            color = "emphasis";
        else
            color = "";
        if (filename[filename.length - 1] == '/') {
            view.attr("filename", filename);
            view.attr("class", 'tabbable folder-item file-item ' + color);
            view.attr("tabIndex", 0);
            view.find(".filename").html(filename.slice(0, filename.length - 1) + " ");
            if (this.childStubs && this.childStubs[filename] && this.childStubs[filename].stub.css("display") != "none")
                view.find(".type-icon").html("folder_open");
            ...
            view.attr("filename", filename);
            view.attr("class", 'tabbable file-item ' + color);
            view.attr("tabIndex", 0);
    ...
    FileBrowser.prototype.getOrCreateView = function(i) {
        ...
        if (this.childViews[i]) {
            ...
            var txt = "<li class=\"file-item\"><span><i class = 'type-icon material-icons'>folder</i></span><span class='filename'>" +
    ...
    FileBrowser.prototype.goto = function(path, cb, isFolder) {
        ...
        if (segments.startsWith(this.fileServer.getRoot())) {
            if (segments.endsWith("/")) {
                ...
                var folder = segments.slice(0, -1).join("/") + "/";
                var self = this;
    ...
    FileBrowser.prototype.openFile = function(filename, forceNew) {
        ...
        configure("recentFolders", recentFolders.join(","), "files");
    ...
    FileBrowser.prototype.showInlineDialog = function(el, callback, status, val) {
        var self = this;
        if (el && !val) {
            val = el.attr("filename").replace("/", "");
        }
        var text = '<div class="renameField">' +
        ...
        if (FileUtils.activeFileBrowser == self)
            self.menu.hide();
    ...
    FileBrowser.prototype.rename = function(former, b) {
        ...
        if (FileUtils.isDirectory(former))
            dest += "/";
        if (dest == path)
            return;
        var self = this;
    ...
    FileBrowser.prototype.childFilePath = function(name, check) {
        if (check && this.hier.indexOf(name) < 0)
            throw "Err: Not Child";
    ...
    FileBrowser.prototype.filename = function(e, notChild) {
        ...
        var name = e.substring(e.lastIndexOf("/") + 1, e.length) + (isFolder ? "/" : "");
        if (!notChild && this.hier.indexOf(name) < 0)
    ...
    FileBrowser.prototype.reload = function(highlightNewFiles, callback, rootDir) {
        ...
        if (this.tree) {
            ...
            this.tree.reload(highlightNewFiles, function(err) {
                ...
                else this.tree.rootDir = self.rootDir;
                callback && callback(err);
            });
            return;
        }
        this.fileServer.getFiles(rootDir || this.rootDir, function(err, res) {
            if (!err) {
        ...
        if (stub.tree) {
            ...
            stub.tree.goto = function(path, cb, i) {
                ...
                if (path.startsWith(stub.rootDir)) {
                    return ChildStub.prototype.goto.apply(stub.tree, [path, cb, i]);
                }
                else {
                    var root = stub.fileServer.getRoot();
                    if (path.startsWith(root)) {
                        var segments = path.substring(root.length).split("/");
                        var max = root + segments.slice(0, (-Math.min(4, segments.length) + 1) || segments.length).join("/");
                        return stub.goto(max + "/", function(e) {
                            if (!e) {
                                ChildStub.prototype.goto.apply(stub.tree, [path, cb, i]);
                            }
                            else cb && cb(e);
                        });
                    }
    ...
    FileBrowser.prototype.onCtxMenuClick = function(id, filename, el) {
        ...
        if (event.defaultPrevented)
            return;
        var i, name, server = stub.fileServer;
        switch (id) {
            case "open-file":
                if (!stub.inSelectMode) {
                    ...
                    var isFolder = FileUtils.isDirectory;
                    for (i in stub.marked) {
                        name = stub.marked[i];
                        if (!isFolder(name)) { //why open multiple folders
                            stub.openFile(name);
                        }
                    }
                }
            ...
            case "show-info":
                ...
                paths.forEach(function(p, i) {
                    ...
                    server.stat(p, function(err, stat) {
                        if (!err) {
                            var size = stat.size;
                            var date = new Date(stat.mtimeMs).toString();
                            total += (size || 0);
                            els.push(table({
                                'Name': stub.marked ? stub.marked[i] : filename,
                                'Byte Length': size,
            ...
            case "new-browser":
                ...
                break;
            case "new-tab":
                FileUtils.initBrowser({ rootDir: filepath || rootDir, server: server });
                break;
            case "copy-file":
                copy_mode = 0;
                fall_through = true;
    ...
    ChildStub.prototype.createHeader = function() {
        return;
        //no headers
    };
    ChildStub.prototype.updateTopElements = function() {
        //do nothing
    };
    ...
    ChildStub.prototype.expandAll = function(callback, depth, eachCallback, accumulator) {
        ...
        function next() {
            ...
            if (self.foldersToIgnore.indexOf(name) < 0)
                self.expandFolder(name, (function(a) {
                    var func = function() {
        ...
        if (segments.startsWith(this.rootDir)) {
            ...
            var next = function(stub, err) {
                ...
                if (segments.length < 1) {
                    ...
                    stub.expandFolder(segments.shift() + "/", next);
                }
            };
            next(this);
        }
    ...
    ChildStub.prototype.expandFolder = function(name, callback) {
        ...
        var callback2 = (function(err) {
            ...
            if (callback)
                callback(this.childStubs[name], err);
        }).bind(this);
        if (el.length < 1) {
    ...
    */
    ...
    Hierarchy.prototype.menuItems["project-dropdown"] = Object.create(ChildStub.prototype.menuItems["childStub-folder-dropdown"]);
