//swipe to open
Modules.SwipeDetector = function(bottomBar, updateBar) {
    "use strict";
    var startTimeT = 0;
    var startX = 0,
        startY = 0,
...
Modules.RepeatDetector = function(mods, appConfig, shortcut_click, tools, handleToolClick, holdKey) {
    "use strict";
    ...
    function startLongPress(e) {
        ...
        if (/a\-/.test(key)) {
            key = key.slice(2);
            func = mods.go;
            targetSpeed = appConfig.characterBarRepeatingKeyInterval;
        }
...
Modules.ScrollLock = function() {
    ...
    function lockScroll() {
        ...
        if (el < right)
            scrollData.detected.scrollLeft = el - scrollData.dir * 2 - 20;
...
//arrow and modkeys
Modules.ModKeyInput = function(FocusManager, toolbar, clipboard) {
    "use strict";
    var times = {};
    ...
    var mod_key_click = function(key, target) {
        ...
        if (clickT - (times[key] || 0) < 700) {
            ...
            if (pressed[key] || held[key] == "double_tap") {
    ...
    function updateModColors(target, key) {
        if (target)
            target.style.color = held[key] ? "red" : pressed[key] || key == 'esc' ? "inherit" : "gray";
        else toolbar.find(".mod-key").each(function() {
...
',' is a special value, use 'comma' instead",
    ...
    var addElement = function(el, above) {
        if (above) {
            ...
            topBar.addClass('top-bar tabbable');
            ...
            if (!bottomBar) {
                ...
                bottomBar.addClass('bottom-bar tabbable');
            }
            bottomEls.push(el);
    ...
    var createToggle = function(bar) {
        ...
        toggle.className = "tool-swapper side-1 tabbable";
    ...
    function resizeWindow() {
        if (global.SplitManager)
            global.SplitManager.notifyResize($(document.body));
        else editor.resize();
    ...
    if (appConfig.enableArrowKeys || appConfig.enableModKeys) {
        ...
        else if (appConfig.enableModKeys == true) {}
        else if (appConfig.enableModKeys == 'toggle') {
            //
        }
        else {
        ...
        if (appConfig.toolbarPosition == "above") {
            modtools = {}
            var metaKeys = ['esc', 'shift', 'ctrl', 'a-left', 'a-down', 'a-up', 'a-right', 'alt'];
    ...
    var toolbar = $(createToolBar(tools));
    var metabar;
    if (modtools) {
        metabar = $(createToolBar(modtools))
    }

    var mods = global.ModKeyInput(FocusManager, metabar || toolbar, clipboard);
    var repeat = global.RepeatDetector(mods, appConfig, shortcut_click, tools, handleToolClick, function(key) {
        FocusManager.hintChangeFocus();
    ...
    var swipe = global.SwipeDetector(bottomBar, function(closed) {
    ...
    if(bottomBar){
        if (!viewToggle) {
            bottomEls[0][0].style.paddingLeft = '40px';
        }
    ...
    global.ScrollLock = global.ModKeyInput = global.RepeatDetector = global.SwipeDetector = null;
