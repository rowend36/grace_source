...
_Define(function(global) {
    ...
    FileBrowser.prototype.createView = function(stub) {
        ...
        if (stub && stub != this.stub) {
            ...
            this.root.html("<h6 class='no-results color-inactive'>Loading files...</h6>");
        }
    ...
    FileBrowser.prototype.updateHierarchy = function(names, highlightNewFiles) {
        if (this.tree) {
            this.updateHeader();
    ...
    FileBrowser.prototype.reload = function(
        ...
        function load(err, res) {
            if (called) throw new Error('getfiles/readdir cb called twice in reload');
            called = true;
            if (!err) {
                self.root.find('.no-results').remove();
                    ...
                    function(e, i, next, cancel) {
                        ...
                        fs.lstat(root + e, function(err, stat) {
                            if (err) {
                                ...
                                if (stat.isDirectory()) {
                                    r[i] += "/";
                                    stats[e + "/"] = toTime(stat.mtime || stat.mtimeMs);
                                } else
                                    stats[e] =
                                    toTime(stat.mtime || stat.mtimeMs) +
                                    "  " +
    ...
    FileBrowser.prototype.onCtxMenuClick = function(id, filename, el) {
        ...
        switch (id) {
            ...
            case "remove-bookmark":
                var index;
                if ((index = FileUtils.getBookmarks().indexOf(rootDir)) > -1) {
            ...
            case "show-info":
                ...
                paths.forEach(function(p, i) {
                    ...
                    server.stat(p, function(err, stat) {
                        if (!err) {
                            var size = stat.size;
                            var date = toTime(stat.mtime || stat.mtimeMs);
                            total += size || 0;
                            els.push(
                                table({
            ...
            case "paste-file":
                ...
                var modal = $(Notify.modal({
                    'header': copy_mode ? "Moving files...." : 'Copying files....',
                ...
                var doPaste = function(path, next, failed) {
                    ...
                    var stop = FileUtils[method + type](
                        ...
                        function(err) {
                            ...
                            if (!err) {
                                if (method == "move") {
                                    if (copyServer == server) {
                                        Docs.rename(path, newpath,
                                            server);
                                    } else Docs.delete(path, server);
                                }
                                next(name);
                            } else {
                                console.error(err);
                        ...
                        function(path, e) {
                            if (e) progress.html('<span class="error-text"> Failed to ' + method +
                                ' ' + path + '</span>');
                ...
                if (fileClipboard.hasOwnProperty('path')) {
                    doPaste(
                        fileClipboard.path,
                        function(name) {
                    ...
                    var filesToCopy = [].concat(fileClipboard.files);
                    ...
                    Utils.asyncForEach(
                        ...
                        function(cancelled) {
                            if (!cancelled)
                                Notify.info("Pasted " + (numCopied || 0) + " files");
                            else if (numCopied > 0)
    ...
    FileBrowser
        .prototype.showCtxMenu = function(menu, el) {
            ...
            if (!fileClipboard) $(document.body).addClass("clipboard-empty");
            else {
    ...
    FileBrowser.prototype.menuItems = {
        "folder-dropdown": {
            ...
            "!update": [function(self, update) {
                update("new-file", FileUtils.inSaveMode() ? self["!save-as"] : self["!new-file"]);
            }]
        },
        "file-dropdown": {
            "open-file": "Open",
        ...
        "create-dropdown": {
            ...
            "!update": [
                function(self, update) {
                    var bookmarked = FileUtils.getBookmarks().indexOf(FileUtils.activeFileBrowser
                        .rootDir) > -1;
                    update(
                        "add-bookmark",
                        bookmarked ?
    ...
    var toggle = function(self, update) {
        ...
        for (var i in self.toggleProps) {
            ...
            var enabled = (filename && data.filename == filename) || (extension && data.extension ==
                extension);
    ...
    function add(menu, id, caption, emitter) {
        ...
        if (caption["!update"]) {
            if (menu["!update"]) menu["!update"].push(caption["!update"]);
    ...
    ChildStub.prototype.setParent = function(browser) {
        ...
        setProp(this, "childFolderDropdown", browser.folderDropdown);
    ...
    ChildStub.prototype.expandFolder = function(name, callback) {
        ...
        var callback2 = function(err) {
            this.extraSpace[this.names.indexOf(name)] = this.childStubs[
                name
            ].stub.height();
    ...
    ChildStub.prototype.onCtxMenuClick = function(id, filename, el) {
        switch (id) {
            case "reload-browser":
                if (filename) {
                    if (this.childStubs[filename]) {
                        this.childStubs[filename].onCtxMenuClick(id, false, el);
                    }
                    return;
                }
                break;
    ...
    ChildStub.prototype
        .menuItems = {
            ...
            "nested-folder-dropdown": {
                ...
                "!update": [function(self, update) {
                    var browser = FileUtils.activeFileBrowser;
                    if (browser.selected && browser.isTree) {
                        browser = browser.childStubs[browser.selected];
                    }
                    update("clear-select", (browser && browser.inSelectMode) ? self[
                        "!clear-select"] : null);
                    update("select-all", (browser && browser
                        .inSelectMode) ? self["!select-all"] : null);
                }]
            },
    ...
    */
    ...
    ProjectBrowser.prototype.reload = function(highlightNewFiles, callback) {
    ...
    Dropdown.assign(ProjectBrowser.prototype.menuItems["project-dropdown"], {
        "open-project": null,
        "close-project": {
    ...
    global.ProjectView = ProjectBrowser;
}); /*_EndDefine*/