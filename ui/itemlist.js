(function(global) {
    ...
    var toString = function(key, ev) {
        return ((ev.ctrlKey || ev.metaKey) ? "ctrl-" : "") + (ev.altKey ? "alt-" : "") + ((ev.shiftKey || /A-Z/.test(key)) ? "shift-" : "") + key.toLowerCase();
    };
    ...
    Utils.extend(ItemList, EventsEmitter);
    ...
    ItemList.prototype.select = function(index) {
        ...
        this.trigger('select', this.getItem(index), true);
    };
    ...
    Utils.extend(QuickList, ItemList);
