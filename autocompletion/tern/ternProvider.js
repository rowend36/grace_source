_Define(function(global) {
    ...
    function onStarted(server) {
        if (!ternServer) {
            ...
            for (var id in docs) {
                if (ternModes[docs[id].options.mode || docs[id].session.getMode().$id])
                    server.addDoc(docs[id].getPath(), docs[id].session);
            }
    ...
    function loadConfig() {
        ...
        Object.assign(ternCompletionProvider.options, {
            ...
            timeout: parseInt(ternConfig.completionTimeout) ||
                (ternConfig.useWebWorkerForTern ? 3000 : 1000),
        });
    }
    loadConfig();
