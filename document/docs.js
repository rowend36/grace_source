_Define(function(global) {
    ...
    configEvents.on("documents", function(ev) {
        switch (ev.config) {
            ...
            case "autosaveInterval":
                autoSave = Utils.delay(Docs.saveDocs, Math.max(Utils.parseTime(
                    appConfig.autosaveInterval), 5000));
        }
    });
    ...
    function updateSizes() {
        ...
        maxDocCacheSize = Math.min(maxStoreSize / 10, Math.max(5000, Utils.parseSize(
            appConfig.maxDocCacheSize)));
        maxDocDataSize = Math.min(maxStoreSize, Math.max(100000, Utils.parseSize(appConfig
            .maxDocDataSize)));
    }
    updateSizes();
    ...
    Docs.tempSave = function(id, force, cleaned) {
        ...
        try {
            ...
            if (data.length > maxDocDataSize) {
                if (!doc.warned)
                    Notify.warn(
                        "The internal data of this file has exceeded the configured limits\n" +
                        doc.getPath() + "\n: " + Math.floor(data.length / 1024) +
                        "kb >" + maxDocDataSize / 1024 + "kb",
    ...
    Docs.saveDocs = function(id, callback, force) {
        if (id !== undefined) {
            if (docs[id].shadowDoc) {
                var mainDoc = Docs.forPath(docs[id].getSavePath(), docs[id]
                    .getFileServer());
    ...
    var autoSave = Docs.$autoSave = Utils.delay(Docs.saveDocs, Math.max(Utils.parseTime(
        appConfig
    ...
    Docs.fromJSON = function(json, ignoreFail, asyncStart) {
        ...
        if (Breaks.hasFailures("loadDocuments") && !ignoreFail) {
        ...
        function loadOne(i, index, next) {
            ...
            if (Breaks.hasFailures(i)) {
                Notify.ask('Proceed to load ' + (json[i] || "Document " + i) + " ?",
                    Docs.$loadData.bind(null, i, json[i], state), Docs.persist);
            } else {
    ...
    function onHandlePreviousFail(json) {
        var el = $(Notify.modal({
            ...
            dismissible: false,
            body: [
                    "<h6>Error during previous load.</br>Choose documents to load</h6><form>"
                ]
                .concat(Object.keys(json).map(function(e) {
                    return "<input data-id='" + e +
                        "' type='checkbox'" + (json[e] ? "" :
                            " checked") + "></input><span>" + (
                            json[e] || "Temporary " + e) +
                        "</span>";
                })).concat(["</form>"]).join(""),
    ...
    Docs.getInfo = function(id) {
        return (docs[id].isTemp() ? "<i>" + docs[id].id + "</i>" : docs[id]
            .getSavePath()) || "<i>" + docs[id].id + "</i>";
