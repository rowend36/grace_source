...
_Define(function(global) {
        ...
        if (!this.mergeTree) {
            ...
            MergeDoc.require(join(dir, gitdir + "-merge"), this.getFileServer(), (function(c) {
                ...
                if ((c && path.startsWith(c.href))) {
                    this.mergeTree = c;
                    Doc.setValue(this, this.getItem().mergedText, cb, force, ignoreDirty);
    ...
    Doc.registerFactory("git-merge", MergeDoc);
    ...
    function previewTree(opts) {
        ...
        //console.log(gatherMods(opts,opts.ours));
        ...
        el.on('click', '.review-btn', (function() {
            var item = conflicts[this.getAttribute("merge-index")];
            var docPath = ourHref + item.path;
            var doc;
            if ((doc = Doc.forPath(docPath))) {
                Doc.swapDoc(doc.id);
                if (!doc.mergeTree) {
                    doc.dir = opts.dir;
        ...
        r.forEach(function(p) {
            ...
            var end = function() {
                if (stub.rootDir == ev.rootDir) {
                    ...
                    prov.status(a).then(update(view));
                }
            };
    ...
    */
    ...
    GitOverflow.close = function(){
        superClose.apply(this,{navigation:true})
    }
    var GitFileOption = {
        caption: "Git...",
        onclick: function(ev) {
