(function(root, factory) {
    ...
    function getDiffs(text1, text2, options, ctx, allowLine) {
        ...
        if (allowLine && !options.showInlineDiffs) {
            ...
            ctx.savedDiffs = (ctx.savedDiffs) ? updateDiff(text1, text2, ctx.swapped, ctx.savedDiffs, getCharDiff) : getCharDiff(text1, text2, false, ctx.swapped);
    ...
    var CellVirusProps = (function() {
        ...
        this.isWidget = function(row, returnRow) {
            ...
            if (mapped > -1) {
                ...
                var next = this.diffs[mapped].leftEndLine;
        ...
        this.$screenToInlineWidgetRow = function(screenRow) {
            ...
            if (widget < 0) {
                ...
                //in a diff
                //is the widget above host diff
                //or below {@see removedFragmentsBelow}
                diff = this.diffs[widget];
                widgetRow = diff.rightStartLine - 1;
                //assuming there is at least one equal line between the two diffs
                var above = diff.leftStartLine - 1;
                if (!widgets || above < 0 || (widgets[above] && widgets[above].type === C.WIDGET_INLINE)) return diff.rightEndLine - 1;
                if (!widgets[row] || widgets[row].type !== C.WIDGET_INLINE) return widgetRow;
                row = above;
            }
            //get the offset of the screen row in the widget
