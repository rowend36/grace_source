_Define(function(global) {
    ...
    var beautify_options = {
        ...
        "wrap_attributes_indent_size": "Indent wrapped attributes to after N characters [indent_size] (ignored if wrap_attributes is \"aligned\")",
        "wrap_line_length": 0,
        "arrowParens": "always",
    ...
    // "templating": undefi
    var appConfig = global.registerAll(beautify_options, "formatting");
    global.registerValues({
        "formatting": "Configure how your code is formatted. camelCase options are supported by Prettier while \nsnake_case_options are used by JsBeautify",
        "autoIndentFallback": "Allows the editor to attempt auto indentation if no formatter is found",
        "brace_style": "collapse_preserve_inline|(collapse|expand|end_expand)[,preserve-inline]|none (default:collapse)",
    ...
    function pretty(val, opts, cb, data) {
        ...
        var pOpts = {
            ...
            printWidth: appConfig.printWidth,
            bracketSpacing: appConfig.bracketSpacing,
            endOfLine: opts.eol,
            tabWidth: opts.indent_size,
            useTabs: opts.indent_char == '\t',
            plugins: prettierPlugins,
            arrowParens: appConfig.arrowParens,
            embeddedLanguageFormatting: appConfig.embeddedLanguageFormatting,
            insertPragma: appConfig.insertPragma && !data.isSegment,
            jsxBracketSameLine: appConfig.jsxBracketSameLine,
            jsxSingleQuote: appConfig.jsxSingleQuote,
            proseWrap: appConfig.proseWrap,
            quoteProps: appConfig.quoteProps,
            semi: appConfig.semi,
            singleQuote: appConfig.singleQuote,
            trailingComma: appConfig.trailingComma,
            vueIndentScriptAndStyle: appConfig.vueIndentScriptAndStyle
        };
        try {
    ...
    var beautifiers = {
        "prettier_js": BootList.define([sharedPrettier, "./tools/format/parser-babel.js"], null, pretty),
        "prettier_css": BootList.define([sharedPrettier, "./tools/format/parser-postcss.js"], null, pretty),
        "prettier_md": BootList.define([sharedPrettier, "./tools/format/parser-markdown.js"], null, pretty),
        "js_beautify": BootList.define(["./tools/format/beautify.min.js"], null, function(val, opts, cb, data) {
            try {
                cb(js_beautify(val, opts), null, data && data.trimmed);
            } catch (e) {
                cb(val, null, false);
                Notify.error(e);
            }
        }),
        "html_beautify": BootList.define([{
            ...
            "./tools/format/beautify-html.min.js"
        ], null, function(val, opts, cb, data) {
            console.log(opts);
            try {
                cb(html_beautify(val, opts), null, data && data.trimmed);
            } catch (e) {
                cb(val, null, false);
                Notify.error(e);
            }
        }),
        "css_beautify": BootList.define(["./tools/format/beautify-css.min.js"], null, function(val, opts, cb, data) {
            try {
                cb(css_beautify(val, opts), null, data && data.trimmed);
            } catch (e) {
                cb(val, null, false);
                Notify.error(e);
            }
    ...
    function setValue(session, value, sel, originalRangeStart, range, formatAll, unselect) {
        ...
        if (typeof value == 'object') {
            if (!value[0] || value[0].start) {
                deltas = value;
            } else deltas = Docs.diffToAceDeltas(value, formatAll ? 0 : originalRangeStart.row, formatAll ? 0 : originalRangeStart.column);
        } else deltas = Docs.generateDiff(formatAll ? session.getValue() : session.getTextRange(range), value, formatAll ? 0 : originalRangeStart.row, formatAll ? 0 : originalRangeStart.column);
        var end = range.start;
    ...
    function beautify(editor, unselect, removeBlanks, overrideOptions) {
        ...
        var beautifier = getBeautifier(detectedMode);
        if (!beautifier) {
            if (appConfig.autoIndentFallback)
