    ...
    (function() {
            ...
            if (start > clipText.length) {
                for (i in tokens) {
                    ...
                    if (char > start) {
                        tokens[i] = {
                            type: tokens[i].type,
                            value: clipText + tokens[i].value.substring(-char + start + tokens[i].value.length)
                        };
                        break;
                    }
            ...
            if (end) {
                ...
                if (char > (start + end)) {
                    tokens[i - 1] = {
                        ...
                        value: tokens[i - 1].value.substring(0, -char + start + end + tokens[i - 1].value.length)
                    };
                }
        ...
        this.render = function(ranges, session, config, parent, maxLine) {
            ...
            for (var rangeId in ranges) {
                ...
                size = (isSingleLine ? range.end.column : line.length) - range.start.column;
        ...
        this.renderPlain = function(ranges, session, config, parent, maxLine) {
            ...
            for (var rangeId in ranges) {
                ...
                parent.append("<div><span id='line-number'></span><span id='start'></span><span class='red-text' id ='result'></span><span id='end'></span></div>");
                ...
                if (lines.length == 1) {
                    ...
                    for (var i = 1; i < lines.length - 1; i++) {
                        elRes.appendChild(document.createTextNode(safe(lines[i])));
                        elRes.innerHTML += "&nbsp;"
                        elRes.appendChild(document.createElement("br"));
                        elRes.innerHTML += "&nbsp;&rdsh;";
                    }
                ...
                parent.children().last().css("font-size", $(".editor").css("font-size"));
            }
        }
        ...
        renderer.$renderLine = function(parent, row, foldLine, multiplexer) {
            ...
            if (foldLine)
                ...
                //the differernce --->
                var tokens = multiplexer(row, this.session.getTokens(row));
